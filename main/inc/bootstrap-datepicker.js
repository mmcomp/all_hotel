                    ...
                    for (var dRow = 0; dRow < numRows; dRow++) { // create date picker rows
                        ...
                        for (var dow = 0; dow < 7; dow++) { // create date picker days
                            ...
                            (otherMonth && !showOtherMonths ? '&#xa0;' : // display for other months
                                (unselectable ? '<span class="ui-state-default">' + printDate.getDate() + '</span>' : ((i.indexOf(printDate.getFullYear()+"-"+(printDate.getMonth()+1)+"-"+printDate.getDate()+"|")<0)?'<a e="'+printDate.getFullYear()+'-'+(printDate.getMonth()+1)+'-'+printDate.getDate()+'" class="ui-state-default':'<a e="'+printDate.getFullYear()+'-'+(printDate.getMonth()+1)+'-'+printDate.getDate()+'" class="ui-state-danger') +
                                    (printDate.getTime() == today.getTime() ? ' ui-state-highlight' : '') +
                                    (printDate.getTime() == currentDate.getTime() ? ' ui-state-active' : '') + // highlight selected day
                                    (otherMonth ? ' ui-priority-secondary' : '') + // distinguish dates from other months
                                    '" onclick="aa(this)" val="'+printDate.getDate()+'" href="#">' + printDate.getDate() + '</a>')) + '</td>'; // display selectable date
