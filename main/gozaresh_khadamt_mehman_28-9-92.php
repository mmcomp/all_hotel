	...
	if(isset($_REQUEST['mod']) and $_REQUEST['mod']==1)
	{
		if((int)$kh_mehman==1)
		{
			...
			if ($nobat!=-1)
				...
				mysql_class::ex_sql("select * from `khadamat_gasht` where `reserve_id`>0 and date(`tarikh`) = date('$tarikh') ",$q);
			while($r=mysql_fetch_array($q))
			{
				$res_id = $r['reserve_id'];			
		...
		elseif ((int)$kh_mehman==4)
		{
			...
			$nobat_kh = '--';
			$kh_tarikh = '--';
			$out = '<table cellpadding="0" cellspacing="0" width="95%" style="font-size:13px;border-style:solid;border-width:1px;border-color:Black;"><tr class="showgrid_header" ><th>ردیف</th><th>سرگروه</th><th>شماره اتاق</th><th>نفرات</th><th>شماره-رزرو</th><th>تلفن</th><th>دفتر</th><th>آژانس</th><th>ورود</th><th>خروج</th><th>نوبت</th><th>تاریخ</th><th>توضیحات</th></tr>';
			if ($nobat!=-1)
...
//echo "select * from `khadamat_akasi` where `reserve_id`>0 and date(`tarikh`) = '$tarikh' ";
			while($r=mysql_fetch_array($q))
			{
				...
				$toz = $r['toz'];		
				$out.="<tr $row_style >";
				$out .="<td class='showgrid_row_td' >$i</td><td class='showgrid_row_td' >".$hotel_tmp->lname."</td><td class='showgrid_row_td' width='30%'>$rooms</td><td class='showgrid_row_td' >$nafar</td><td class='showgrid_row_td' >".$r['reserve_id']."</td><td class='showgrid_row_td' >".$hotel_tmp->tozih."</td><td class='showgrid_row_td' >".$daftar->name."</td><td class='showgrid_row_td' >".$ajans->name."</td><td class='showgrid_row_td'>$room_aztarikh1</td><td class='showgrid_row_td'>$room_tatarikh1</td><td class='showgrid_row_td' >$nobat_name</td><td class='showgrid_row_td' >$kh_tarikh</td><td class='showgrid_row_td' >$toz</td>";
				$out.='</tr>';
				$i++;	
			}
			$out .="</table><br/>\n";
