<?php
	...
	if( isset($_REQUEST['h_id']))
		...
		$tarikh = $tmp[0];
                $frm="";
		$sum = 0;
		$out = '<table cellpadding="0" cellspacing="0" width="95%" style="border-style:solid;border-width:1px;border-color:Black;"><tr class="showgrid_header" ><th>ردیف</th><th>نام سرگروه</th><th>نفرات</th><th>شماره-رزرو</th><th>اطلاعات بیشتر</th><th>دفتر</th><th>آژانس</th><th>وضعیت</th><th>تعداد</th><th width="40%">توضیحات</th></tr>';
		$q=null;
		...
		while($r=mysql_fetch_array($q))
		{
			...
			*///-----------------------------------------------
			...
			$out.="<tr $row_style >";
			$out .="<td class='showgrid_row_td' >$i</td><td class='showgrid_row_td' >".$hotel_tmp->lname."</td><td class='showgrid_row_td' >$nafar</td><td class='showgrid_row_td' >".$r['reserve_id']."</td><td class='showgrid_row_td' >".$hotel_tmp->tozih."</td><td class='showgrid_row_td' >".$daftar->name."</td><td class='showgrid_row_td' >".$ajans->name."</td><td class='showgrid_row_td' >$status</td><td class='showgrid_row_td' >".$r['tedad']."</td><td class='showgrid_row_td' width='40%'></td>";
			$out.='</tr>';
		}
		$out.='<tr class="showgrid_row_odd" ><td colspan="2" style="text-align:left;">جمع نفرات : </td><td align="center"  >'.$nafar_kol.'</td><td colspan="5" style="text-align:left;">جمع : </td><td align="center"  >'.$sum.'</td><td></td></tr>';
		...
		if( $conf->cost_control && ($khad->typ==0 && $sum>0 && ($se->detailAuth('all') || $se->detailAuth('anbar_dari'))) )
		{
			...
			if($r = mysql_fetch_array($q))
				$jam_kol = (int)$r['jam'];
			$out .="<tr class='showgrid_row_even' ><td style='text-align:left;'>خروج از انبار : </td><td>".loadAnbar()."</td><td>تحویل گیرنده:</td><td>".loadUsers()."</td><td align='left' >تعداد:</td><td><input name='cost_tedad' id='cost_tedad' class='inp' style='width:40px;' value='$cost_tedad' onkeypress='return numbericOnKeypress(event);' >از: ".($sum - $jam_kol)."</td><td colspan='4'>".loadKalaTarkibi()."<input type='button' value='چاپ رسید خروج از انبار' class='inp' $disable onclick='send_info($khadamat_id,$sum);'>$pm</td></tr>\n";
			$out .="</table><br/>\n";
			//--------------------------------
