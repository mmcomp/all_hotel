	...
	if(isset($_REQUEST['mod']) && (int)$_REQUEST['mod']==2)
	{
		...
		for($i=0;$i<count($tmp);$i++)
		{
			...
			for($j=0;$j<count($room['rooms']);$j++)
			{
				...
				if (($day == date("Y-m-d",strtotime($room['rooms'][$j]['tatarikh'])))&&(reserve_class::isKhorooj($tmp[$i],$room['rooms'][$j]['room_id'])))
					$rooms.=' <span style="color:#ffffff;background:#0c5e06"> '.$tmp_room->name.'</span>'.(($j<count($room['rooms'])-1)?' , ':'');
				else
