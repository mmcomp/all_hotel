<?php
	...
	if(isset($_REQUEST['rep']) && ((int)$_REQUEST['rep']==1) && ($rooms_ids!=""))
	{
		if ($rm_names!=-1)
			...
			$room_shart = "";
		$output = '<br/><table border="1" cellpadding="0" cellspacing="0" width="80%" style="font-size:12px;border-style:solid;border-width:1px;border-color:Black;" ><tr class="showgrid_header" ><th>کاربر رزرو گیرنده </th><th>نام </th><th>شماره رزرو</th><th>شماره اتاق</th><th>تاریخ ورود</th><th>تاریخ خروج</th><th>ساعت ورود</th><th>ساعت خروج</th></tr>';
		if(isset($_REQUEST['mod']) && (int)$_REQUEST['mod']==2)
		{
			...
			while($r = mysql_fetch_array($tmphelp))
			{
				...
				if($i%2==0)
				 ...
				 $output .="<tr $row_style><td>$reserver_user</td><td>$fulname</td><td>$reserve</td><td>$room</td><td>$den</td><td>$ta</td><td>$vorood_h</td><td>$vh</td></tr>";
				$i++;
			}	
	...
	{
		///nezafat
	$output = '<br/><table border="1" cellpadding="0" cellspacing="0" width="80%" style="font-size:12px;border-style:solid;border-width:1px;border-color:Black;" ><tr class="showgrid_header" ><th>ردیف</th><th>شماره اتاق</th><th>کاربر ثبت کننده</th><th>نام میهمان</th><th>شماره رزرو</th><th>تاریخ ثبت </th><th>ساعت ثبت</th><th>کاربر نظافت کننده</th><th>تاریخ نظافت </th><th>ساعت نظافت</th><th>وضعیت</th></tr>';
		...
		while($r = mysql_fetch_array($q))
		{
			...
			elseif ($r['en']==1)
				...
				$stat = 'نامشخص';
			$output .="<tr $row_style><td>$i</td><td>$room_name</td><td>$user_sabt</td><td>$name_mehman</td><td>$res_id</td><td>$aztarikh_tb</td><td>$saat</td><td>$user_nezafat</td><td>$tarikh_nezafat</td><td>$saat_n</td><td>$stat</td></tr>";
			$i++;
		}
