<?php
		...
		////gasht
			...
			$comb_nobat = "<select class='inp' id=\"nobat\" name=\"nobat\"  style=\"width:auto;\">\n<option value=\"-1\">\n&nbsp\n</option>\n";
				...
				elseif((int)$nobat==2)
					...
					echo '';
				$comb_nobat .= "<option value='1' $comboTyp_sobh>نوبت صبح</option>\n";
				$comb_nobat .= "<option value='2' $comboTyp_asr>نوبت عصر</option>\n";
			$comb_nobat .= "</select>";
		}
		elseif ($kh_mehman==3)
		{	
	...
	if(isset($_REQUEST['mod']) and $_REQUEST['mod']==1)
	{
		if((int)$kh_mehman==1)
		{
			...
			$tatarikh = date("Y-m-d");
			if ($nobat>0)
				mysql_class::ex_sql("select `reserve_id` from `khadamat_gasht` where `reserve_id`>0 and date(`tarikh`) = '$tarikh' and `typ`='$nobat'",$q);
			else
				mysql_class::ex_sql("select `reserve_id` from `khadamat_gasht` where `reserve_id`>0 and date(`tarikh`) = '$tarikh'",$q);
			$tmp ='';
			while ($r = mysql_fetch_array($q))
			{
			...
			if ($tmp!='')
				...
				$shart_res = "1=0 and ";
			mysql_class::ex_sql("select `id` from `khadamat` where `name`='گشت' and `hotel_id`='$hotel_id_new' order by `name`",$q);
			if ($r = mysql_fetch_array($q))
				$khadamat_id = $r['id'];
			else
				$khadamat_id = -1;
			$out = '<table cellpadding="0" cellspacing="0" width="98%" style="font-size:13px;border-style:solid;border-width:1px;border-color:Black;"><tr class="showgrid_header" ><th>ردیف</th><th>سرگروه</th><th>شماره اتاق</th><th>نفرات</th><th>شماره-رزرو</th><th>تلفن</th><th>دفتر</th><th>آژانس</th><th>تعداد</th><th>ورود</th><th>خروج</th><th>نوبت</th><th>تاریخ</th></tr>';
			...
			while($r=mysql_fetch_array($q))
			{
				...
				mysql_class::ex_sql("select `tarikh`,`typ` from `khadamat_gasht` where `reserve_id`>0 and date(`tarikh`) = '$tarikh' and `reserve_id`='$res_id'",$q_kh);
				if($r_kh=mysql_fetch_array($q_kh))
				{
					$kh_tarikh = audit_class::hamed_pdate(date("Y-m-d",strtotime($r_kh['tarikh'])));
					if ($r_kh['typ']==1)
						$nobat_kh = 'نوبت صبح';
					elseif ($r_kh['typ']==2)
						$nobat_kh = 'نوبت عصر';
				...
				$out.="<tr $row_style >";
				$out .="<td class='showgrid_row_td' >$i</td><td class='showgrid_row_td' >".$hotel_tmp->lname."</td><td class='showgrid_row_td' width='30%'>$rooms</td><td class='showgrid_row_td' >$nafar</td><td class='showgrid_row_td' >".$r['reserve_id']."</td><td class='showgrid_row_td' >".$hotel_tmp->tozih."</td><td class='showgrid_row_td' >".$daftar->name."</td><td class='showgrid_row_td' >".$ajans->name."</td><td class='showgrid_row_td' >".$r['tedad']."</td><td class='showgrid_row_td'>$room_aztarikh1</td><td class='showgrid_row_td'>$room_tatarikh1</td><td class='showgrid_row_td' >$nobat_kh</td><td class='showgrid_row_td' >$kh_tarikh</td>";
				$out.='</tr>';
				$i++;
			}
			$out .="</table><br/>\n";
		...
		elseif((int)$kh_mehman==2)
		{
			...
			if ($r = mysql_fetch_array($q))
				...
				$khadamat_id = -1;
			$out = '<table cellpadding="0" cellspacing="0" width="100%" style="font-size:13px;border-style:solid;border-width:1px;border-color:Black;"><tr class="showgrid_header" ><th>ردیف</th><th>سرگروه</th><th>شماره اتاق</th><th>نفرات</th><th>شماره-رزرو</th><th>تلفن</th><th> ورود</th><th>خروج</th><th>نوع</th><th>راننده</th><th>مبدا/مقصد</th><th>تاریخ</th></tr>';
			if(isset($khadamat_id) && isset($tarikh))
		...
		elseif ((int)$kh_mehman==3)
		{
			...
			$tatarikh = date("Y-m-d");
			if ($nobat>0)
				mysql_class::ex_sql("select `reserve_id` from `khadamat_cinema` where `reserve_id`>0 and date(`tarikh`) = '$tarikh' and `typ`='$nobat'",$q_trans);
			else
				mysql_class::ex_sql("select `reserve_id` from `khadamat_cinema` where `reserve_id`>0 and date(`tarikh`) = '$tarikh'",$q_trans);
			$tmp ='';
			while ($r_trans = mysql_fetch_array($q_trans))
			{
				$r_hotel = room_class::loadHotelByReserve($r_trans['reserve_id']);
			...
			if ($tmp!='')
				...
				$shart_res = "1=0 and ";
			mysql_class::ex_sql("select `id` from `khadamat` where `name`='سینما' and `hotel_id`='$hotel_id_new' order by `name`",$q);
			if ($r = mysql_fetch_array($q))
				$khadamat_id = $r['id'];
			else
				$khadamat_id = -1;
			$out = '<table cellpadding="0" cellspacing="0" width="95%" style="font-size:13px;border-style:solid;border-width:1px;border-color:Black;"><tr class="showgrid_header" ><th>ردیف</th><th>سرگروه</th><th>شماره اتاق</th><th>نفرات</th><th>شماره-رزرو</th><th>تلفن</th><th>دفتر</th><th>آژانس</th><th>تعداد</th><th>ورود</th><th>خروج</th><th>نوبت</th><th>تاریخ</th></tr>';
			if(isset($khadamat_id) && isset($tarikh))
				mysql_class::ex_sql("select * from `khadamat_det` where  $shart_res `khadamat_id`=$khadamat_id order by `reserve_id`",$q);
			else
				mysql_class::ex_sql("select * from `khadamat_det` where `reserve_id`>0 and 1=0",$q);
			while($r=mysql_fetch_array($q))
			{
				...
				if($i%2==0)
					$row_style = 'class="showgrid_row_even"';
				mysql_class::ex_sql("select `tarikh`,`typ` from `khadamat_cinema` where `reserve_id`>0 and date(`tarikh`) = '$tarikh' and `reserve_id`='$res_id'",$q_kh);
				if($r_kh=mysql_fetch_array($q_kh))
				{
					$kh_tarikh = audit_class::hamed_pdate(date("Y-m-d",strtotime($r_kh['tarikh'])));
					if ($r_kh['typ']==1)
		...
		elseif ((int)$kh_mehman==4)
		{
			...
			$tarikh = date("Y-m-d");
			if ($nobat>0)
				mysql_class::ex_sql("select `reserve_id` from `khadamat_akasi` where `reserve_id`>0 and date(`tarikh`) = '$tarikh' and `typ`='$nobat'",$q_trans);
			else
				mysql_class::ex_sql("select `reserve_id` from `khadamat_akasi` where `reserve_id`>0 and date(`tarikh`) = '$tarikh'",$q_trans);
			$tmp ='';
			while ($r_trans = mysql_fetch_array($q_trans))
			{
				$r_hotel = room_class::loadHotelByReserve($r_trans['reserve_id']);
			...
			if ($tmp!='')
				...
				$shart_res = "1=0 and ";
			mysql_class::ex_sql("select `id` from `khadamat` where `name`='عکاسخانه' and `hotel_id`='$hotel_id_new' order by `name`",$q);
			if ($r = mysql_fetch_array($q))
				$khadamat_id = $r['id'];
			else
				$khadamat_id = -1;
			...
			while($r=mysql_fetch_array($q))
			{
				...
				if($i%2==0)
					$row_style = 'class="showgrid_row_even"';
				mysql_class::ex_sql("select `tarikh`,`typ` from `khadamat_akasi` where `reserve_id`>0 and date(`tarikh`) = '$tarikh' and `reserve_id`='$res_id'",$q_kh);
				if($r_kh=mysql_fetch_array($q_kh))
				{
					$kh_tarikh = audit_class::hamed_pdate(date("Y-m-d",strtotime($r_kh['tarikh'])));
					if ($r_kh['typ']==1)
...
گزارش خدمات ارائه شده به میهمان
	...
	<body>
		...
		<div align="center">
			...
			<table id='search_tb' width="50%" style='font-size:14px;font-weight:bold;'>
				<tr valign="bottom" >
					<td>	نام هتل
					</td>
					<td>	
