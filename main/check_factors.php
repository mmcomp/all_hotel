<?php	session_start();
	...
	if(isset($_REQUEST['print']))
	{
		...
		$factors = array();
		$inserted=array();
		//echo "SELECT *  FROM `sandogh_factor` WHERE date(`tarikh`)='$date1' and `tedad`!='0' and `en`='1' $whereClause";
		mysql_class::ex_sql("SELECT *  FROM `sandogh_factor` WHERE date(`tarikh`)='$date1' and `tedad`!='0' and `en`='1' $whereClause",$que1);
		//mysql_class::ex_sql("SELECT *  FROM `sandogh_factor` WHERE `factor_shomare` = 1",$que1);
		...
		{	
			...
			$name=(isset($mehman[0]))?$mehman[0]->fname .' ' .$mehman[0]->lname:'----';
...
<html>
	...
	<style>
		...
		function pre(ou)
		{
			var tb="<tr><td><table  height='200' align='left' dir='rtl' border='1'>";
			tb+="<tr><td colspan='12'  align='center'><h1> فاکتور مشتری </h1></td></tr>";
			tb+="<tr><td  align='center'><h3> تاریخ: </h3></td><td align='center' colspan='10' >"+$("#tim").html()+"</td></tr>";
			tb+="<tr align='center'><td nowrap> نام مشتری: </td><td align='center'>"+ou[0].customer_name+"</td>";
			tb+="<td nowrap>  شماره رزرو:  </td><td colspan='3' align='center'>"+ou[0].reserve_id+"</td></tr>";
			tb+="<tr><td nowrap>شماره فاکتور:   </td><td align='center'>"+ou[0].shomare+"</td>";
			tb+="<td nowrap>  شماره اتاق:  </td><td colspan='3' align='center'>"+ou[0].room_name+"</td></tr>";
			tb+="<tr><td align='left'>شماره میز</td><td colspan='3'>&nbsp;</td></tr>";
			tb+="<tr align='center'><td> ردیف  </td>";
			tb+="<td colspan='2' nowrap> لیست غذا  </td>";
			tb+="<td>  تعداد   </td>";
			//tb+="<td nowrap>  قیمت غذا   </td>";
			//tb+="<td  nowrap colspan='2'> جمع </td></tr>";
			for(var i=0;i<ou.length;i++)
			{
				tb+="<tr align='center'><td>"+(i+1)+"</td>";
				tb+="<td colspan='2' nowrap>"+ou[i].food_name+"</td>";
				tb+="<td>"+ou[i].food_num+"</td>";
				//tb+="<td>"+ou[i].food_cost+"</td>";
				//tb+="<td colspan='2' >"+ou[i].food_num*ou[i].food_cost+"</td></tr>";
