<?php
	...
	function loadKhadamat($hotel_id)
	{
		...
		for($i=0;$i<count($kh );$i++)
		{
			...
			$khorooji = '';
			if($kh[$i]['voroodi'])
				$voroodi = "اول<input type='checkbox' name='kh_v_".$kh[$i]['id']."' >";
			if($kh[$i]['khorooji'])
				$khorooji = "آخر<input type='checkbox' name='kh_kh_".$kh[$i]['id']."' >";
			$inp = $inp = 	"<input type='checkbox' name='kh_ch_".$kh[$i]['id']."' > $voroodi $khorooji";
			if($kh[$i]['typ']==0)
			{
				$inp = 	"تعدادروزانه:<input type='text' class='inp' style='width:30px;' name='kh_txt_".$kh[$i]['id']."' value='".((isset($_REQUEST['kh_'.$kh[$i]['id']]))?$_REQUEST['kh_'.$kh[$i]['id']]:0)."' > $voroodi $khorooji";
			}
	...
	if($hotel_id>0 && count($rooms)>0)
	{
		...
		if ($hotel->hotelAvailableBetween($aztarikh,$tatarikh))
		{
			...
			$output .= "<tr><td colspan='3' >مبلغ تور:<input onkeyup='monize(this);' class='inp' type='text' name='tour_mablagh' id='tour_mablagh' value='".((isset($_REQUEST['tour_mablagh']))?$_REQUEST['tour_mablagh']:"")."' ></td><td>مبلغ بلیت:<input onkeyup='monize(this);' class='inp' type='text' name='belit_mablagh' id='belit_mablagh' value='".((isset($_REQUEST['belit_mablagh']))?$_REQUEST['belit_mablagh']:"")."' ></td></tr>";
			...
			$output .= "</tr>";
			$sel_ajBelit = ((isset($_REQUEST['ajans_idBelit']))?$_REQUEST['ajans_idBelit']:0);
			$output .= "<tr><td>نام شرکت بلیت</td><td>".loadDaftarBelit($daftar_idBelit)."</td><td>نام آژانس</td><td colspan='1' >". loadAjansBelit($daftar_idBelit,$sel_ajBelit)."</td>";
			$output .= "</tr>";
			$output .= "</tr>";
			...
			$output .= "<tr><td colspan='4' ><input type='button' onclick=\"if(radioChecked()){document.getElementById('frm1').action='reserve2.php';document.getElementById('frm1').submit();}else{alert('لطفاً اطلاعات را کامل وارد کنید');}\" value='رزرو'  class='inp' ></td></tr>";
	...
	<head>
		...
		<script  type="text/javascript" >	
			...
			function send_search()
			{
				...
				if(document.getElementById('daftar_idBelit')) document.getElementById('daftar_idBelit').selectedIndex = -1;
				if(document.getElementById('ajans_idBelit')) document.getElementById('ajans_idBelit').selectedIndex = -1;
