<?php
	...
	function register_reserve($ws_user,$ws_pass,$ajans_id,$aztarikh,$shab,$room_id,$tedad,$khadamat,$fname,$lname,$tel,$user_id,$extra_toz,$aj_id=-1)
        {
                $out = null;
                $conf = new conf;
                if($conf->checkWsdl($ws_user,$ws_pass))
			...
			{
				$sanad_recs = room_det_class::preReserve($room->hotel_id,$ajans_id,(is_array($room_id))?$room_id:array($room_id),$ghimat,$aztarikh,$shab,1,FALSE,FALSE,$tedad,$khad,$user_id);
				if($sanad_recs !== FALSE)
				{
					$toz["toz"] = $tel;
	...
	$server->register('load_reserves',array('ws_user'=>'xsd:string','ws_pass'=>'xsd:string','tarikh'=>'xsd:string','ajans_id'=>'xsd:int','key'=>'xsd:int'),array('return'=>'xsd:string'),
            ...
            'encoded',
            'Load All Reserves  . . .');
        $server->register('register_reserve',array('ws_user'=>'xsd:string','ws_pass'=>'xsd:string','ajans_id'=>'xsd:int','aztarikh'=>'xsd:string','shab'=>'xsd:int','room_id'=>'xsd:string','tedad'=>'xsd:int','khadamat'=>'xsd:string','fname'=>'xsd:string','lname'=>'xsd:string','tel'=>'xsd:string','user_id'=>'xsd:int','extra_toz'=>'xsd:string','aj_id'=>'xsd:int'),array('return'=>'xsd:string'),
            'test_wsdl',
