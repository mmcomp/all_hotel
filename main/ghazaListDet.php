...
*/
	...
	function delete_item($id)
		...
		mysql_class::ex_sql("select `khadamat_det_id` from `khadamat_det_front` where `id` = '$id'",$q);
		if($r = mysql_fetch_array($q))
		{
			$id_kh = $r['khadamat_det_id'];
			mysql_class::ex_sql("select `reserve_id` from `khadamat_det` where `id` = '$id_kh'",$q_kh);
			if($r_kh = mysql_fetch_array($q_kh))
			{
				$res = $r_kh['reserve_id'];
				mysql_class::ex_sqlx("INSERT INTO `user_DelGhaza` (`id`, `user_id`, `reserve_id`, `tarikh`) VALUES (NULL, '$user_id', '$res', '$today')");
			}
		}
		mysql_class::ex_sqlx("delete from `khadamat_det_front` where `id` = $id");
        }
	$out = 'خطا';
