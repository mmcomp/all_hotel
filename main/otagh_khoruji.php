<?php
	...
	{
		$today = date("Y-m-d");
		mysql_class::ex_sql("select `id`,`room_id`,`reserve_id` from `room_det` where `id`='$inp'",$q);
		if($r = mysql_fetch_array($q))
		{
			if (reserve_class::isKhorooj($r['reserve_id'],$r['room_id']))
				$out = "<span style='background-color:#b72b13;'>"."خارج شده"."</span>";
			else
				$out = "<span style='background-color:#0c5e06;'>"."خارج نشده"."</span>";
		}		
		return $out;
	}
