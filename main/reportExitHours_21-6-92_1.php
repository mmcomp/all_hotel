<?php
	...
	if(isset($_REQUEST['rep']) && ((int)$_REQUEST['rep']==1) && ($rooms_ids!=""))
	{
		if ($rm_names!=-1)
			...
			$room_shart = "";
		$output = '<br/><table border="1" cellpadding="0" cellspacing="0" width="80%" style="font-size:12px;border-style:solid;border-width:1px;border-color:Black;" ><tr class="showgrid_header" ><th>نام </th><th>شماره رزرو</th><th>شماره اتاق</th><th>تاریخ ورود</th><th>تاریخ خروج</th><th>ساعت ورود</th><th>ساعت خروج</th></tr>';
	...
	elseif(isset($_REQUEST['rep']) && ((int)$_REQUEST['rep']==2) && ($rooms_ids!=""))
	{
		...
		mysql_class::ex_sql("SELECT * FROM  `nezafat` WHERE $room_shart DATE(`mani_time`) >='$tatarikh' and DATE(`mani_time`) <='$tatarikh2'order by `mani_time`",$tmphelp);
		$i = 1;
		while($r = mysql_fetch_array($tmphelp))
		{
			$row_style = 'class="showgrid_row_odd"';
