<?php
	...
	function edit_item($id,$feild,$value)
	{
		...
		if($feild=='en')
		{
			mysql_class::ex_sql("select `toz_fix` from `tasisat_tmp` where `id`='$id'",$q);
			if($r = mysql_fetch_array($q))	
			{
				if ($r['toz_fix']!='')
					mysql_class::ex_sqlx("update `tasisat_tmp` set `user_fixed`='$user_id',`en`='1',`date_fix`='$today' where `id`=$id ");
				else
					echo '<br/><center><h3>'.'لطفا برای رفع مشکل توضیح را وارد نمایید'.'</h3></center>';
			}
		}
		elseif($feild=='toz')
		{
		...
		{
			mysql_class::ex_sqlx("update `tasisat_tmp` set `toz_fix`='$value' where `id`=$id ");
		}
		else
			$c = '1=1';
	}
	...
	$grid->columnAccesses[9] = $se->detailAuth('modir');
