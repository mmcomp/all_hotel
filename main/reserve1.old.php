<?php
	...
	function loadKhadamat($hotel_id)
	{
		...
		for($i=0;$i<count($kh );$i++)
		{
			...
			$inp = 	"<input $class $view style='display:none;' type='checkbox' name='kh_ch_".$kh[$i]['id']."' id='kh_ch_".$kh[$i]['id']."' > $voroodi $khorooji";
			if($kh[$i]['typ']==0)
			{
				$inp = 	"تعدادروزانه:<input $class type='text' class='inp' style='width:30px;' name='kh_txt_".$kh[$i]['id']."' id='kh_txt_".$kh[$i]['id']."'  value='".((isset($_REQUEST['kh_'.$kh[$i]['id']]))?$_REQUEST['kh_'.$kh[$i]['id']]:0)."' > $voroodi $khorooji";
			}
		...
		if ($hotel->hotelAvailableBetween($aztarikh,$tatarikh))
		{
			...
			if($conf->room_select)
			{
				...
				for($i=0;$i<sizeof($rooms);$i++)
				{
					...
					$output .= "<td><input style='display:none;' class='inp' name='room_gh_".$rooms[$i]['room_typ_id']."' value='".$rooms[$i]['ghimat']."' > </td></tr><tr id='tr_$i' style='display:".showHide($room_numbers).";' >";
				...
				//
				$output .= "<tr><td colspan='3'><input type='button' value='انتخاب همه اتاقها' class='inp' onclick=\"select_all_rooms('otagh');\" ></td>";
			}
			...
			$canEdit = '';
			if($conf->ghimat_readonly=='TRUE' && !($se->detailAuth('all') || $se->detailAuth('reserve')))
				$canEdit = 'readonly="readonly"';
			$output .= "<tr><td colspan='3' >$tour_mab_view<input onkeyup='monize(this);' class='inp' type='text' name='tour_mablagh' id='tour_mablagh' $canEdit value='".((isset($_REQUEST['tour_mablagh']))?$_REQUEST['tour_mablagh']:"")."' ></td><td>$m_belit1_view<input onkeyup='monize(this);' class='inp' type='text' name='belit_mablagh_1' id='belit_mablagh_1' value='".((isset($_REQUEST['belit_mablagh_1']))?$_REQUEST['belit_mablagh_1']:"")."' ><br/>$m_belit2_view<input $m_belit2_style onkeyup='monize(this);' class='inp' type='text' name='belit_mablagh_2' id='belit_mablagh_2' value='".((isset($_REQUEST['belit_mablagh_2']))?$_REQUEST['belit_mablagh_2']:"")."' ></td></tr>";
			$output .= "</tr>";
			$jcheck = 'radioChecked';
			if($conf->room_select)
				$jcheck = 'checkboxChecked';
			$output .= "<tr><td colspan='4' >
<input type='button' class='inp' onclick=\"send_calc();if($jcheck()){getAjaxInfo();}else{document.getElementById('wait').style.display='none';alert('لطفاً اطلاعات را کامل وارد کنید');}\" value='محاسبه قیمت'>
<img src='../class/wait.gif' style='display:none;' id='wait' >
<input type='hidden' name='ghimat_calc' id='ghimat_calc' value=0 >
<input type='button' onclick=\"document.getElementById('ghimat_calc').value=0;if($jcheck()){getAjaxInfo();}else{alert('لطفاً اطلاعات را کامل وارد کنید');}\" value='رزرو' id='reserve_but'  class='inp' ><input value='$tmp_check' name='room_id_tmp' id='room_id_tmp' type='hidden' ></td></tr>";
			...
			<table border='1' >
				...
				<tr>
					...
					<td>
						...
						<input type="text" class="inp" style="width:30px;" name="shab" id="shab" value="<?php  echo ((isset($_REQUEST['shab']))?$_REQUEST['shab']:1); ?>"  >
						<?php 
						}
						else 
						{	
