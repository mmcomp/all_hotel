...
//echo $khad[$i]['id'].','.$khad[$i]['typ'].','.$khadamat[$j]['voroodi'].'<br/>';
			...
			$gahst_tarikh = '';
			$kh_id = $khad[$i]['id'];
			mysql_class::ex_sql("select `gashtAst` from `khadamat` where `id` = '$kh_id'",$p);
			...
			if($i%2 == 0)
				$out .= "<tr><td colspan='2' style='border-style:solid;border-width:1px;' >$inp $voroodi $khorooji $gahst_tarikh</td>";
			else
				$out .= "<td colspan='2' style='border-style:solid;border-width:1px;' >$inp $voroodi $khorooji $gahst_tarikh</td></tr>";
...
//var_dump($khadamat_sabti);
$ta_gasht = ((isset($_REQUEST['ta_gasht']))?audit_class::hamed_pdateBack($_REQUEST['ta_gasht']):'');
$tmp_gasht = explode(' ',$ta_gasht);
			$reserveid_shomaresanad = room_det_class::reReserve($reserve_id,$h_id,$reserve->hotel_reserve->ajans_id,$tmp_room,$ghimat,$aztarikh,$shab,$tedad_otagh,$shab_reserve,$rooz_reserve,$tedad_nafarat,$khadamat_sabti,$tmp_gasht[0]);
		...
		if($h_id>0 && isset($_REQUEST['s_reserve_id']) )
		{
			...
			if($bool!==FALSE)
			{
				...
				*/
				...
				$output .='<tr><th colspan="5" style="border-style:dashed;border-width:1px;" >خدمات</th></tr>'.loadKhadamat($h_id,$reserve->khadamat_det,$reserve_id);
				$output .='</table>';
				$output .= "</td>";
				$output .= '</tr>';
