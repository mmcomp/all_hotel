	...
	if( isset($_REQUEST['sargrooh_id']))
	{
		...
		if ($comboTyp==1)
		{
			...
			if ($r = mysql_fetch_array($q))
			{
				...
				mysql_class::ex_sqlx("INSERT INTO `khadamat_transfer` (`id`, `khadamat_id`, `reserve_id`, `driverName`, `target_id`, `timeKh`,`typ`) VALUES (NULL, '$khadamat_id', '$sargrooh_id', '$nameDriver', '$target_id', '$time_kh','$comboTyp')");
			}
		}
		elseif ($comboTyp==2)
		{
...
///////START sargrooh
	...
	mysql_class::ex_sql("select `reserve_id`,`id` from `khadamat_det` where `reserve_id`>0 and DATE(`tarikh`) = '$tarikh' and `khadamat_id`=$khadamat_id order by `reserve_id`",$q);
	while($r = mysql_fetch_array($q))
	{
...
///////END Target
	...
	$out = '<table cellpadding="0" cellspacing="0" width="95%" style="border-style:solid;border-width:1px;border-color:Black;"><tr class="showgrid_header" ><th>ردیف</th><th>نام سرگروه</th><th>شماره اتاق</th><th>نفرات</th><th>شماره-رزرو</th><th>اطلاعات بیشتر</th><th>تاریخ ورود</th><th>تاریخ خروج</th><th>نوع</th><th>نام راننده</th><th>مبدا/مقصد</th><th>ساعت</th><th>وضعیت</th></tr>';
	if(isset($khadamat_id) && isset($tarikh))
			mysql_class::ex_sql("select * from `khadamat_det` where `reserve_id`>0 and DATE(`tarikh`) = '$tarikh' and `khadamat_id`=$khadamat_id order by `reserve_id`",$q);
		else
		...
		<div align="center" id="div_main" >
			...
			<br/>	
				<form id='frm2' method='GET'>
					...
					نام راننده
					<input type='text' name='nameDriver' id='nameDriver' value="<?php echo$nameDriver;?>">
					مسیر:<?php echo $comboTarget;?>	
					زمان حرکت:
					<input type='text' name='transferTime' id='transferTime' value="<?php echo $transferTime?>"/>
					<input type='hidden' name='h_id' id='h_id' value="<?php echo $h_id?>"/>
					<input type='hidden' name='tarikh' id='h_id' value="<?php echo $tarikh?>"/>
					نوع
					<?php echo $comboTyp;?>
