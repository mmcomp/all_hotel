...
/////END HOTEL
	...
	if( isset($_REQUEST['sargrooh_id']))
	{
		...
		if( isset($_REQUEST['transferTime']))
			...
			$transferTime = -1;
		if ($_REQUEST['mablagh_naghdi']=='')
		{
			mysql_class::ex_sql("select `hazine` from `target` where `id`='$target_id'",$q);
			if ($r = mysql_fetch_array($q))
				$hazine = $r['hazine'];
			else
				$hazine = '100';
			...
			$hazine = $_REQUEST['mablagh_naghdi'];
		...
		if ($comboTyp==1)
		{
			...
			if ($r = mysql_fetch_array($q))
			{
				...
				$sanad_driver = sanadzan_class::sanad_transfer($shomare_sanad,$nameDriver,$sargrooh_id,$user_id,$hazine);
			}
		}
		elseif ($comboTyp==2)
		{
...
///////START sargrooh
	...
	{
		$res_id = $r['reserve_id'];
		$typ_mehman = room_det_class::loadTypByReserve($res_id,$tarikh);
		mysql_class::ex_sql("select * from `khadamat_transfer` where `reserve_id`='$res_id' and `khadamat_id`=$khadamat_id and date(`timeKh`)='$tarikh' order by `reserve_id`",$q_tra);
		if (!($r_tra = mysql_fetch_array($q_tra)))
		{
			...
			$combo_sargrooh .= $hotel_tmp->lname.$typ_mehman."\n";
...
///////START Target
	...
	$comboTarget .= "<select class='inp' id=\"target_id\" name=\"target_id\"  style=\"width:auto;\">\n<option value=\"-1\">\n&nbsp\n</option>\n";
		mysql_class::ex_sql("select * from `target` order by `name`",$q);
		while($r = mysql_fetch_array($q))
		{
			if((int)$r["id"]== (int)$target_id)
			        ...
			        $select = "";
	        $comboTarget .= "<option value=\"".(int)$r["id"]."\" $select   >\n";
	        $comboTarget .= $r["name"]."\n";
	        $comboTarget .= "</option>\n";
		}
	$comboTarget .= "</select>";
///////END Target
	...
	$comboTyp .= "</select>";
	$select = "";
	$comboDriver = "<select class='inp' id=\"nameDriver\" name=\"nameDriver\" onchange=\"showNaghdi();\" style=\"width:auto;\">\n<option value=\"-1\">\n&nbsp\n</option>\n";
	...
	$comboDriver .= "</select>";
///////////////////////////DELETE
	if ((isset($_REQUEST['mod_del'])) and ($_REQUEST['mod_del']=='del'))
	{
		$id_tra_del =((isset($_REQUEST['id_del']))?($_REQUEST['id_del']):-1);
		if ($id_tra_del!=-1)
		{
			$shomare_sanad = sanadzan_class::getShomareSanad();
			$sanad_driver = sanadzan_class::delete_sanad_transfer($shomare_sanad,$id_tra_del,$user_id);
			mysql_class::ex_sqlx("DELETE FROM `khadamat_transfer` WHERE `id` = '$id_tra_del'");
			echo "<script>alert('خدمات مورد نظر کنسل شد');</script>";
		}
		else
			echo "<script>alert('برای میهمان مورد نظر ترانسفری ثبت نشده است');</script>";
	}
//////////////////////////EndDELETE
	...
	$out = '<table cellpadding="0" cellspacing="0" width="100%" style="border-style:solid;border-width:1px;border-color:Black;"><tr class="showgrid_header" ><th>ردیف</th><th>نام سرگروه</th><th>شماره اتاق</th><th>نفرات</th><th>شماره-رزرو</th><th>اطلاعات بیشتر</th><th>تاریخ ورود</th><th>تاریخ خروج</th><th>نوع</th><th>نام راننده</th><th>مبدا/مقصد</th><th>تاریخ</th><th>وضعیت</th><th>توضیحات</th><th>کنسل</th></tr>';
	...
	while($r=mysql_fetch_array($q))
	{
		...
		//{
			...
			$out .="<td class='showgrid_row_td' >$i</td><td class='showgrid_row_td' >".$hotel_tmp->lname."</td><td class='showgrid_row_td' width='30%'>$rooms</td><td class='showgrid_row_td' >$nafar</td><td class='showgrid_row_td' >".$r['reserve_id']."</td><td class='showgrid_row_td' >".$hotel_tmp->tozih."</td><td class='showgrid_row_td'>$room_aztarikh1</td><td class='showgrid_row_td'>$room_tatarikh1</td><td class='showgrid_row_td'>$status</td><td class='showgrid_row_td' >$nameDriver</td><td class='showgrid_row_td' >$target</td><td class='showgrid_row_td' >$transferTime</td><td class='showgrid_row_td' >$stat_kh</td><td class='showgrid_row_td' >$toz_tra</td><td class='showgrid_row_td' ><input type='button' value='کنسل' class='inp' onclick='deleteTransfer($kh_transfer_id);' ></td>";
			$out.='</tr>';
			$i++;
	//	}
	}
...
ثبت خدمات ترانسفر برای میهمان
	...
	<body>
		...
		<div align="center" id="div_main" >
			...
			<form id='frm_del' method='get'>
				<input type='hidden' name='id_del' id='id_del' />
				<input type='hidden' name='mod_del' id='mode_del' value='del' />
			...
			<br/>	
				<form id='frm2' method='GET'>
					...
					مبلغ
					<input style='display:none;' type='text' name='mablagh_naghdi' id='mablagh_naghdi' />
