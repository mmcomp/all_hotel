...
//-------------------------------------------------------------------------------------------------------------------------
	...
	function kill_reserve($ws_user,$ws_pass,$reserve_id)
        ...
        function reserve_verify($ws_user,$ws_pass,$reserve_id,$hotel_ir_id,$comision)
        {
                $out = FALSE;
                $conf = new conf;
                if($conf->checkWsdl($ws_user,$ws_pass))
			...
			if($r = mysql_fetch_array($q))
			{
				...
				$re = new hotel_reserve_class;
				$re->loadByReserve($reserve_id);
				$ghimat = $re->m_hotel;
				$comision = (($comision != '')?unserialize($comision):0);
				if(is_array($comision) && $comision !== 0)
					if(reserve_class::isPick($reserve_id) && isset($comision['pick']))
						$comision = (float)$comision['pick'];
					else if(!reserve_class::isPick($reserve_id) && isset($comision['nopick']))
						$comision = (float)$comision['nopick'];
					else
						$comision = 0;
				else
					$comision = 0;
				$msql->ex_sqlx("insert into `hotel_ir_reserve` (`moshtari_id`, `reserve_id`, `ghimat`, `tarikh`, `hotel_ir_id`, `comision`) values (".((int)$conf->getMoshtari()).",$reserve_id,$ghimat,'$tarikh',$hotel_ir_id,'$comision')");
				$out = TRUE;
			}
                }
                return($out);
	...
	function idToName($ws_user,$ws_pass,$tbname,$id)
        ...
        function addPardakht($ws_user,$ws_pass,$value,$mablagh)
        {
                $out = null;
                $conf = new conf;
                if($conf->checkWsdl($ws_user,$ws_pass))
                {
			$no = date("Y-m-d H:i:s");
			$out = pardakht_class::add($value,$no,$mablagh);
                }
                return($out);
        }
	...
	$server->register('addPardakht',array('ws_user'=>'xsd:string','ws_pass'=>'xsd:string','value'=>'xsd:int','mablagh'=>'xsd:int'),array('return'=>'xsd:int'),
            'test_wsdl',
            'test_wsdl#addPardakht',
            'rpc',
            'encoded',
            'Add a new Pardakht and returns it`s id.');
...
//sanad_gozaresh($ws_user,$ws_pass,$ajans_id,$aztarikh,$tatarikh,$frase)
	$server->register('sanad_gozaresh',array('ws_user'=>'xsd:string','ws_pass'=>'xsd:string','ajans_id'=>'xsd:int','aztarikh'=>'xsd:string','tatarikh'=>'xsd:string','frase'=>'xsd:string'),array('return'=>'xsd:string'),
        ...
        $server->register('reserve_tmpList',array('ws_user'=>'xsd:string','ws_pass'=>'xsd:string'),array('return'=>'xsd:string'),
            ...
            'encoded',
            'Lists Temp Reserves . . .');
        $server->register('reserve_verify',array('ws_user'=>'xsd:string','ws_pass'=>'xsd:string','reserve_id'=>'xsd:int','hotel_ir_id'=>'xsd:int','comision'=>'xsd:int'),array('return'=>'xsd:boolean'),
            'test_wsdl',
