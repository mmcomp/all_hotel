<?php
//Direct to ne
header('Location: rooms_vaziat.php');
exit();
//----------------
...
//var_dump($_SESSION);
	...
	$tmp2=array('main_param'=>array('dafater'=>null,'agency'=>null,'user_manage'=>null,'hozoor_ghiab'=>null,'m_hotel'=>null,'hotel_daftar'=>null,'watcher'=>null,'mehman_sms'=>null,'paziresh_info'=>null,'ticket'=>null,'enteghad_modir'=>null,'shenavar'=>null),'accu'=>array('h_kol'=>null,'h_moeen'=>null,'sanad_new'=>null,'sanad_daftar'=>null,'sanad_belit'=>null,'hesab_close'=>null,'taraz'=>null,'bank'=>null));
        ...
        $tmp2['onUser'] = array('listUser'=>null,'listInOut'=>null,'showLog'=>null);
	...
	$tmp2['main_param']['sabtVade'] = null;
	$tmp2['main_param']['garantiTabagh'] = null;
...
<head>
	...
	function locateMsgDiv()
	{
		...
		$.get("index.php?payam=payam",function(result){
			msgDiv.hide();
			...
			tedadMsgDiv.html(result+" پیام خوانده نشده");
...
<body style="width:100%;height:100%;">
	...
	/*	$msgs = msg_class::loadMsgs($user_id);
		...
		$typ_user = '-1';
		$typ_user = user_class::loadTyp($user_id);
		if (($typ_user=='4')||($typ_user=='13')||($typ_user=='14')||($typ_user=='16'))
		{
/*
			$yesterday = date('Y-m-d',strtotime("-1 days"));
			mysql_class::ex_sql("select `id` from `gozareshRozane` where `user_id`='$user_id' and `tarikh`='$yesterday'",$qu);
			if($r_g = mysql_fetch_array($qu))
				echo '';
			else
			{
				echo "<script>alert('گزارش روز قبل توسط نام کاربری شما ثبت نگردیده است. لطفا ابتدا گزارش روز قبل خود را ثبت و سپس دوباره وارد شوید');</script>";
				die("<script>window.location = 'gozaresh_roozane.php?user_id=$user_id&tarikh=$yesterday&';</script>");
			}
*/
		}		
	?>
