<?php
	...
	mysql_class::ex_sql("select `nafar`,`reserve_id`,`tatarikh` from `room_det` ",$qr);
	...
	while($rr = mysql_fetch_array($qr))
	{
		...
		if (($tmp_tatarikh == $day1)&&($res>0)&&(!reserve_class::isKhorooj($res)))
			$today_khoruj ++;
	}
	...
	$q=null;
	mysql_class::ex_sql("select count(`id`) as `tedad` from `mehman` where `room_id` in ($tedad_mehman) and `khorooj`='0000-00-00 00:00:00'",$q);
	while($r = mysql_fetch_array($q))
		$tedad_mehman_moghim = $r['tedad']."<br/>";
	$combo = "";
		...
		<div align="center">
			...
			<table >
				<tr>
					...
					<td style="background-color:#a38fb3;	-webkit-border-radius: 8px; -moz-border-radius: 8px; border-radius: 8px; border:1px solid black;padding:5px;">تعداد اتاق های خارج از سرویس</td>
			...
			<table>
				<tr>
					...
					<td style="background-color:#a38fb3;	-webkit-border-radius: 8px; -moz-border-radius: 8px; border-radius: 8px; border:1px solid black;">خارج از سرویس</td>
