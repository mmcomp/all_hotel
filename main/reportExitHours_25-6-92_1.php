<?php
	...
	$output = '<br/><table border="1" cellpadding="0" cellspacing="0" width="80%" style="font-size:12px;border-style:solid;border-width:1px;border-color:Black;" ><tr class="showgrid_header" ><th>ردیف</th><th>کاربر ثبت کننده</th><th>نام میهمان</th><th>شماره رزرو</th><th>تاریخ ثبت </th><th>ساعت ثبت</th><th>کاربر نظافت کننده</th><th>تاریخ نظافت </th><th>ساعت نظافت</th><th>وضعیت</th></tr>';
		mysql_class::ex_sql("select * from `nezafat` where date(`mani_time`)>= '$tatarikh' and date(`mani_time`)<='$tatarikh2' order by `mani_time`,`en`",$q);
		$i =1;
		while($r = mysql_fetch_array($q))
		{
			...
			elseif ($r['en']==1)
				...
				$stat = 'نامشخص';
			$output .="<tr $row_style><td>$i</td><td>$user_sabt</td><td>$name_mehman</td><td>$res_id</td><td>$aztarikh_tb</td><td>$saat</td><td>$user_nezafat</td><td>$tarikh_nezafat</td><td>$saat_n</td><td>$stat</td></tr>";
			$i++;
		}
