...
/////END HOTEL
	$now = date("Y-m-d");
	mysql_class::ex_sql("select `id` from `khadamat` where `name`='عکاسخانه' and `hotel_id`='$h_id' order by `name`",$q);
	...
	if( isset($_REQUEST['sargrooh_id']))
	{
		...
		mysql_class::ex_sqlx("INSERT INTO `khadamat_akasi` (`id`, `khadamat_id`, `reserve_id`, `typ`,`tarikh`,`toz`) VALUES (NULL, '$khadamat_id', '$sargrooh_id', '$comboTyp','$tarikh','$toz')");
...
///////START sargrooh
	...
	mysql_class::ex_sql("select `reserve_id`,`id` from `room_det` where `reserve_id`>0 and ((date(`aztarikh`) < '$aztarikh' and date(`tatarikh`) > '$aztarikh')) group by `reserve_id`",$q);
	while($r = mysql_fetch_array($q))
	{
		$res_id = $r['reserve_id'];
		mysql_class::ex_sql("select * from `khadamat_akasi` where `reserve_id`='$res_id' and `khadamat_id`='$khadamat_id'",$q_gasht);
...
///////END sargrooh
	...
	$comboTyp = "<select class='inp' id=\"comboTyp\" name=\"comboTyp\"  style=\"width:auto;\">\n<option value=\"-1\">\n&nbsp\n</option>\n";
		...
		while($r = mysql_fetch_array($q))
		{
			if((int)$r["id"]== (int)$comboTyp)
			        $select = "selected='selected'";
			else
			        $select = "";
			$comboTyp .= "<option value=\"".(int)$r["id"]."\" $select   >\n";
			$comboTyp .= $r["name"]."\n";
			$comboTyp .= "</option>\n";
		}
	$comboTyp .= "</select>";
