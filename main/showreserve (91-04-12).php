<?php
	...
	//-----newend------
		...
		if($h_id>0 && isset($_REQUEST['reserve_id']) && $_REQUEST['mod']==301 )
		{
			...
			{
				$ghimat = array();
				$ghimat['ghimat_tour'] = $m_belit_sabti_1 +$m_belit_sabti_2+ $m_hotel_sabti;
			...
			//echo "aztarikh=$aztarikh<br/>tatarikh=$tatarikh";
			...
			{
				room_det_class::sabtReserveHotel($reserve_id,$reserveid_shomaresanad['shomare_sanad'],$ghimat,'',$lname,$toz,$reserve->hotel_reserve->ajans_id,$m_hotel_sabti,$reserve->hotel_reserve->regdat,TRUE);
		...
		if($h_id>0 && isset($_REQUEST['s_reserve_id']) )
		{
			...
			if($bool!==FALSE)
			{
				...
				$output .='<tr><th>نام و نام خانوادگی سرگروه</th><th>توضیحات</th><th>آژانس رزرو کننده</th><th>'.$m_belit_view.'</th><th>مبلغ هتل</th></tr>';
				...
				$output .= "<td>$m_belit1_view<input $ghimat_disable class='inp' type='text' name='m_belit_1' id='m_belit_1' value='".$reserve->hotel_reserve->m_belit1."' onblur=\"checkNumber(this);\" ><input  style='display:none;' type='text' id='mirror_m_belit_1' value='".$reserve->hotel_reserve->m_belit1."' >$m_belit2_view<input $m_belit2_style $ghimat_disable class='inp' type='text' name='m_belit_2' id='m_belit_2' value='".$reserve->hotel_reserve->m_belit2."' onblur=\"checkNumber(this);\" ><input style='display:none;' type='text' id='mirror_m_belit_2' value='".$reserve->hotel_reserve->m_belit2."' ></td>";
				...
				*/
				...
				$output .="<tr><td>$m_belit1_view ".loadDaftarBelit($req_daftar_id_1,1).loadDaftarBelitMirror($belit_ghabli_tmp['daftar_belit1'],1)."</td><td>".loadAjansBelit($req_daftar_id_1,$sel_aj_1,1).loadAjansBelitMirror($belit_ghabli_tmp['daftar_belit1'],$belit_ghabli_tmp['ajans_belit1'],1)."</td>";
				$output .="<td><input type='checkbox' name='shab_reserve' id='shab_reserve' ".loadShabReserve($reserve->room_det[0]->aztarikh)." > <input style='display:none;' type='checkbox' id='mirror_shab_reserve' ".loadShabReserve($reserve->room_det[0]->aztarikh)." > </td>";
				$output .="<td><input type='checkbox' name='rooz_reserve' id='rooz_reserve' ".loadRoozReserve($reserve->room_det[0]->tatarikh)." > <input  style='display:none;' type='checkbox' id='mirror_rooz_reserve' ".loadRoozReserve($reserve->room_det[0]->tatarikh)." ></td></tr>";

				$output .="<tr $m_belit2_style ><td>برگشت:".loadDaftarBelit($req_daftar_id_2,2).loadDaftarBelitMirror($belit_ghabli_tmp['daftar_belit2'],2)."</td><td>".loadAjansBelit($req_daftar_id_2,$sel_aj_2,2).loadAjansBelitMirror($belit_ghabli_tmp['daftar_belit2'],$belit_ghabli_tmp['ajans_belit2'],2)."</td>";
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script  type="text/javascript" >
			...
			function submit_frm()
			{
				if( (parseInt(document.getElementById('m_belit_1').value,10)==0 && document.getElementById('daftar_idBelit_1').selectedIndex >0) || (parseInt(document.getElementById('m_belit_1').value,10)>0 && document.getElementById('daftar_idBelit_1').selectedIndex <=0) || (parseInt(document.getElementById('m_belit_2').value,10)==0 && document.getElementById('daftar_idBelit_2').selectedIndex >0) || (parseInt(document.getElementById('m_belit_2').value,10)>0 && document.getElementById('daftar_idBelit_2').selectedIndex <=0) ||  (parseInt(document.getElementById('m_hotel').value,10)==0) || (parseInt(document.getElementById('m_belit_1').value,10)!=0 && document.getElementById('ajans_idBelit_1').selectedIndex<0) || (parseInt(document.getElementById('m_belit_2').value,10)!=0 && document.getElementById('ajans_idBelit_2').selectedIndex<0)  )
				{
	...
	<body>
		...
		<div align="center">
			...
			<?php 
				...
				else if(isset($reserve) && $reserve->isOnline)
