<?php
	...
	if(isset($_GET["check"]) && (int)$_GET["check"]==2  )
	{
		...
		if($kol_id != -1 && $moeen_id == -1)
		{
			...
			$hamed_wer = "`kol_id` = '$kol_id' and  en=1 group by `moeen_id`";
			$qq = null;
		...
		{
			$hamed_wer = "`kol_id` = '$kol_id' and `moeen_id`=$moeen_id and en=1 group by `moeen_id`";
			$qq = null;
		...
		{
			$hamed_wer = "en=1 group by `kol_id` order by `kol_id`";
			$qq = null;
