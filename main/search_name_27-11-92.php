<?php
	...
	*/
	...
	if($se->detailAuth('super'))
		...
		$output = '<br/><table border="1" cellpadding="0" cellspacing="0" width="80%" style="font-size:12px;border-style:solid;border-width:1px;border-color:Black;" ><tr class="showgrid_header" ><th>شماره رزرو</th><th>حساب</th><th>هتل</th><th>آژانس</th><th>نام</th><th>شماره اتاق</th><th>تعداد نفرات</th><th>قیمت هتل</th><th>جمع کل</th><th>تاریخ ورود</th><th>تاریخ خروج</th></tr>';
	...
	if(isset($_REQUEST['mod']) && (int)$_REQUEST['mod']==2)
	{
				...
				if (($rooms_ids!='')&&($rooms_ids!=')'))
				{
					...
					if($r_res = mysql_fetch_array($q_res))
					{
						...
						else
						{
							...
							$output .="<td>".$room['rooms'][0]['hotel']."</td><td>$ajName</td><td>".$name[0]."</td><td>$rooms</td><td>".$room['rooms'][0]['nafar']."</td><td>".monize($horel_reserve->m_hotel)."</td>";
		...
		if(!($se->detailAuth('super')))
			$output .="<tr $styl ><td>جمع</td><td>--</td><td>--</td><td>--</td><td>$sum_room</td><td>$nafar</td><td>$mablagh</td><td>$mablagh_kol</td><td>--</td><td>--</td><td>--</td></tr>";
	}
	$output .='</table>';
?>
		...
		<center>
			<table>
				<tr>
					...
					<td style="background-color:#009999;	-webkit-border-radius: 8px; -moz-border-radius: 8px; border-radius: 8px; border:1px solid black;">گارانتی</td>
				</tr>
			</table>
		</center>
	</body>
