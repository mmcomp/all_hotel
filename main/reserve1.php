...
// 			mysql_class::ex_sql('select `id`,`name` from `daftar` where `id`='.$inp.' and `kol_id`>0 order by `name`',$q);
		mysql_class::ex_sql('select `id`,`name` from `daftar` where `kol_id`>0'.(($_SESSION['daftar_id']!=49)?' and id not in (48,49) '/*' and id = '.$_SESSION['daftar_id']*/:'').' order by `name` ',$q);
		while($r = mysql_fetch_array($q))
		{
			$sel = (($r['id']==$inp)?'selected="selected"':'');
			$out.="<option $sel  value='".$r['id']."' >".$r['name']."</option>\n";
	...
	if ($mode1!=1)
  {
				...
				if(count($rooms)){
					foreach($rooms as $room){
						...
						if($hotel->is_shab_nafar!=1){
							...
							$room['ghimat'] = $yeknafarshab*$rt->zarfiat;
		...
		if($hotel_id>0 && count($rooms)>0)
		{
			...
			if ($hotel->hotelAvailableBetween($aztarikh,$tatarikh))
			{
				...
				else
				{
					...
					for($i=0;$i<sizeof($rooms);$i++)
					{
						...
						for($j=0;$j<count($room_numbers);$j++)
						{
							...
							if($rooms[$i]['zarfiat_ezafe']>0){
								$tmp_ezafe = (isset($_REQUEST['ezafe_otagh_'.$room_numbers[$j]]) && $check_box!='')?$_REQUEST['ezafe_otagh_'.$room_numbers[$j]]:0;
								if(isset($_REQUEST['ezafe_otagh_'.$room_numbers[$j]]) && (int)$tmp_ezafe>0){
									$tmp_check_ezafe[] = $room_numbers[$j].'|'.$tmp_ezafe;
								}
								$output .= '<select id="ezafe_otagh_'.$room_numbers[$j].'" name="ezafe_otagh_'.$room_numbers[$j].'" style="font-size: 8px" >';
								for($ci = 0;$ci <= $rooms[$i]['zarfiat_ezafe'];$ci++){
									$output .= '<option value="'.$ci.'"'.(($tmp_ezafe==$ci)?' selected':'').'>'.$ci.'</option>';
								}
								$output .= '</select>';
							}
							$output .= '</span>';
				...
				$output .= "<tr style='display:none'><td>دفتر </td><td>".loadDaftarBelit($daftar_idBelit_1,1)."</td><td>$raft_sherkat</td><td colspan='1' >". loadAjansBelit($daftar_idBelit_1,$sel_ajBelit_1,1)."</td>";
  			$output .= "</tr>";
	...
	<span style=\"margin:5px;\" class=\"btn btn-info col-md-4\" onclick=\"send_calc();if($jcheck()){getAjaxInfo();}else{document.getElementById('wait').style.display='none';alert('لطفاً اطلاعات را کامل وارد کنید');}\">

	محاسبه قیمت</span>
	<img src='../class/wait.gif' style='display:none;' id='wait' >
	<input type='hidden' name='ghimat_calc' id='ghimat_calc' value=0 >
	<a style=\"margin:5px;\" id='reserve_but' class=\"btn btn-success col-md-4\" onclick=\"document.getElementById('ghimat_calc').value=0;if($jcheck()){getAjaxInfo();}else{alert('لطفاً اطلاعات را کامل وارد کنید');}\">

	رزرو</a>
	<input value='".((count($tmp_check_ezafe)>0)?implode(',',$tmp_check_ezafe):'')."' name='room_id_tmp_ezafe' id='room_id_tmp_ezafe' type='hidden' >
	<input value='$tmp_check' name='room_id_tmp' id='room_id_tmp' type='hidden' ></td></tr>";
        ...
        $output="<input type=\"hidden\" id=\"di1\">";
    }
$root="";
?>
<!DOCTYPE html>
    ...
    <script type="text/javascript" src="../js/tavanir.js"></script>
      <script>
          ...
          var check_otagh = false;
			...
			function mehrdad_ajaxFunction(fn,command,hotel_id,aztarikh,shab,nafar,room,daftar_id,ajans_id,room_ezaf){
				/*
				...
				*/
				...
				if(room_ezaf){
					p['rooms_ezf'] = room_ezaf;
				}
// 				console.log("ajax_responser.php",p);
				jQuery.get("ajax_responser.php",p,function(result){
// 					console.log('a');
...
// 				console.log('is_pick');
				...
				var nafar = parseInt(document.getElementById('tedad_nafarat').value,10);
				var room = document.getElementById('room_id_tmp').value;
				var room_ezaf = document.getElementById('room_id_tmp_ezafe').value;
				var daftar_id = <?php echo $daftar_id; ?>;
				...
				if(ajans_id>0)
				{
					...
					$.get("ajax_responser.php?r="+Math.random()+"&command=khghimat&hotel_id="+hotel_id+"&test_date="+aztarikh+"&delay="+shab+"&rooms="+room+"&nafar="+nafar+"&daftar_id="+daftar_id+"&ajans_id="+ajans_id+'&kh='+jkh+'&',function(result){
						...
						mehrdad_ajaxFunction(check_ghimat,'getghimat',hotel_id,aztarikh,shab,nafar,room,daftar_id,ajans_id,room_ezaf);
					});
...
*/
			...
			function send_calc()
			{
				...
				document.getElementById('tour_mablagh').value=0;
