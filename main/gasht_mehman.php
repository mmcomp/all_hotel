...
/////END HOTEL
	...
	if( isset($_REQUEST['sargrooh_id']))
	{
		...
		mysql_class::ex_sqlx("INSERT INTO `khadamat_gasht` (`id`, `khadamat_id`, `reserve_id`, `typ`,`tarikh`) VALUES (NULL, '$khadamat_id', '$sargrooh_id', '$comboTyp','$tarikh')");
...
///////START sargrooh
	$aztarikh = date("Y-m-d");
	$tatarikh = date("Y-m-d");
	mysql_class::ex_sql("select `reserve_id` from `room_det` where `reserve_id`>0 and ((date(`aztarikh`) < '$aztarikh' and date(`tatarikh`) > '$aztarikh')) group by `reserve_id`",$q);
