<?php
	...
	function loadKhadamat($hotel_id)
	{
		...
		for($i=0;$i<count($kh );$i++)
		{
			...
			if($kh[$i]['voroodi'])
				$voroodi = "اول<input  $jtick type='checkbox' name='kh_v_".$kh[$i]['id']."' id='kh_v_".$kh[$i]['id']."' >";
			if($kh[$i]['khorooji'])
				$khorooji = "آخر<input $jtick type='checkbox' name='kh_kh_".$kh[$i]['id']."' id='kh_kh_".$kh[$i]['id']."' >";
			$inp = 	"<input style='display:none;' type='checkbox' name='kh_ch_".$kh[$i]['id']."' id='kh_ch_".$kh[$i]['id']."' > $voroodi $khorooji";
			if($kh[$i]['typ']==0)
			{
	...
	*/
	...
	if($hotel_id>0 && count($rooms)>0)
	{
		...
		if ($hotel->hotelAvailableBetween($aztarikh,$tatarikh))
		{
			...
			$output .="<td >تعدادنفرات:<input type='text' id='tedad_nafarat' name='tedad_nafarat' value='".((isset($_REQUEST['tedad_nafarat']))?$_REQUEST['tedad_nafarat']:1)."' class='inp' ></td></tr>";
			...
			$output .= "<tr><td colspan='4' ><input type='button' onclick=\"if($jcheck()){getAjaxInfo();}else{alert('لطفاً اطلاعات را کامل وارد کنید');}\" value='رزرو' id='reserve_btn'  class='inp' ><input value='$tmp_check' name='room_id_tmp' id='room_id_tmp' type='hidden' ></td></tr>";
