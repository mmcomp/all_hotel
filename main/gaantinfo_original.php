<?php
	...
	if(isset($_REQUEST['set_room_id']))
	{
		$out = 'false';
		$set_room_id = (int)$_REQUEST['set_room_id'];
		$set_reserve_id = (int)$_REQUEST['set_reserve_id'];
		$tedad = (int)$_REQUEST['tedad'];
		if($tedad>0)
			$out = mysql_class::ex_sqlx("update room_det set nafar = $tedad where room_id=$set_room_id and reserve_id = $set_reserve_id");
		die($out);
	}
...
//echo '2';
	...
	{		
		...
		if (!$se->detailAuth('resturant'))
		{
			...
			$paziresh ="<td><a target='_blank' href='paziresh.php?reserve_id=$reserve_id_code&room_id=$room_id&kh=0' >پذیرش</a>&nbsp;<a style='color:red;' target='_blank' href='gaantinfo_nafar.php?reserve_id=$reserve_id' >ویرایش نفرات</a>&nbsp;$khorooj</td>";
		}
		else
		{
			$khorooj = '';
		...
		if(($_SESSION['daftar_id']==$ajans->daftar_id || $isAdmin))
		{
			...
			if(isset($room['rooms']))
			{
				for($j=0;$j<count($room['rooms']);$j++)
				{
					...
					$rooms.='<span style="cursor:pointer;" >'.$tmp_room->name.'['.$room['rooms'][$j]['nafar'].']</span>'.(($j<count($room['rooms'])-1)?' , ':'');
				}
				...
				if(!$is_khorooj)
				{
					if (count($room['rooms'])>0)
					{
						...
						if ($se->detailAuth('modir'))
						{
							...
							$output .="<td>".$room['rooms'][0]['hotel']."</td><td>$aj_name</td><td>".$name[0]."</td><td>$rooms</td><td>".$nafar."</td><td>".monize($horel_reserve->m_hotel)."</td>";
						...
						if($horel_reserve->extra_toz!='')
							$output .="<tr $styl ><td>توضیحات : </td><td colspan='9'>".$horel_reserve->extra_toz.'<br/>'.' ساعت ورود:'.$voroodH."</td><td></td><td></td><td></td></tr>";
						else
							$output .="<tr $styl ><td>توضیحات : </td><td colspan='9'> ساعت ورود:".$voroodH."</td><td></td><td></td><td></td></tr>";
					}
			}
		}
		else
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function setNafar(room_id,rreserve_id)
		{
			var a = prompt('تعداد نفرات این اتاق را وارد کنید');
			if(a)
				$.get("gaantinfo.php",{"set_room_id":room_id,"set_reserve_id":rreserve_id,"tedad":a},function(result){
					if(result == 'ok')
...
alert(result);
	...
	<body>
		<div style="margin:10px;">
			...
			<a href="ravabet.php?room_id=<?php echo $room_id;?>&reserve_id=<?php echo $reserve_id;?>&" target="_blank"><img title="نظرسنجی" src="../img/nazar.png" width="50px"/></a>			
		</div>
                <?php echo security_class::blockIfBlocked($se,lang_fa_class::block); ?>
		<div align="center">
			<h3><?php echo $vazeat_name;?></h3>
			<br/>
			<table border='0' style="font-family: Terafik,Tahoma,tahoma;font-size:12px;<?php echo ($isTasisat)?'display:none;':''; ?>">
				<tr>
					<td>
					...
					<td>
						<img style="cursor:pointer" onclick="<?php echo $poshtiban_onclick;?>" src="../img/poshteban.png" title="پشتیبان"/><br/>پشتیبان
					</td>
					<td>
						<img style="cursor:pointer" onclick="<?php echo $tamir_onclick;?>" src="../img/tasisat.png" title="در دست تعمیر"/><br/>در دست تعمیر
					</td>
				</tr>
			</table>
			<input id="vaziat" class="inp" name="vaziat" value="<?php echo $vaziat; ?>" style="width:40px;" type="hidden">			
				...
				<?php
					$tas = <<< TTS
						<h2>تاسیسات</h2>
						<iframe style="border:none;" src="fixProb.php?hotel_id=$hotel_id&room_id=$room_id&" width="100%"></iframe>

TTS;
					if(($isAdmin || $se->detailAuth('super')) && $vaziat == 4)
						echo '<a href="tasisat.php?room_id='.$room_id.'&" target="_blank">تاسیسات</a>';
					echo $output.'</table>'.$tas.'<br/><h3>اطلاعات پذیرش</h3>'.$out;
				?>
		</div>
	</body>
</html>
