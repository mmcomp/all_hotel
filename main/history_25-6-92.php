<?php
	...
	$out_2 .= '<br/><table border="1" cellpadding="0" cellspacing="0" width="80%" style="font-size:12px;border-style:solid;border-width:1px;border-color:Black;" ><tr class="showgrid_header" ><th>ردیف</th><th>کاربر ثبت کننده</th><th>نام میهمان</th><th>شماره رزرو</th><th>تاریخ</th><th>وضعیت</th></tr>';
		...
		while($r = mysql_fetch_array($q))
		{
			...
			$out_2 .="<tr $row_style><td>$i</td><td>$user_sabt</td><td>$name_mehman</td><td>$res_id</td><td>$aztarikh_tb</td><td>نظافت نشده</td></tr>";
			$i++;
		}
	...
	$out_4 .= '<br/><table border="1" cellpadding="0" cellspacing="0" width="80%" style="font-size:12px;border-style:solid;border-width:1px;border-color:Black;" ><tr class="showgrid_header" ><th>ردیف</th><th>کاربر ثبت کننده</th><th>توضیحات</th><th>تاریخ ثبت</th><th>کاربر رفع کننده مشکل</th><th>توضیحات</th><th>تاریخ رفع مشکل</th><th>وضعیت</th></tr>';
		...
		while($r = mysql_fetch_array($q))
		{
			...
			if ($r['date_fix']!='0000-00-00 00:00:00')
				...
				$answerdate = '--';
			$out_4 .="<tr $row_style><td>$i</td><td>$user_sabt</td><td>$toz</td><td>$tarikh_sabt</td><td>$user_answer</td><td>$answer</td><td>$answerdate</td><td>$stat</td></tr>";
			$i++;
		}
