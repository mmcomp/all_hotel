<?php
	...
	if( isset($_REQUEST['h_id']))
		...
		while($r=mysql_fetch_array($q))
		{
			...
			$ghaza = "";
			$r_res = $r['reserve_id'];
			mysql_class::ex_sql("select `id` from `khadamat_det` where `khadamat_id` = $khadamat_id and `reserve_id`='$r_res'",$q1);
		...
		$b = array();
		$i = 1;
		mysql_class::ex_sql("select `id` from `khadamat_det` where `khadamat_id` = $khadamat_id ",$q1);
		while($r1= mysql_fetch_array($q1))
		{
			$kh_det_id = $r1["id"];
			$b[] = $kh_det_id;
