...
/////END HOTEL
	$now = date("Y-m-d");
	mysql_class::ex_sql("select `id` from `khadamat` where `name`='سینما' and `hotel_id`='$h_id' order by `name`",$q);
	...
	if( isset($_REQUEST['sargrooh_id']))
	{
		...
		mysql_class::ex_sqlx("INSERT INTO `khadamat_cinema` (`id`, `khadamat_id`, `reserve_id`, `typ`,`tarikh`) VALUES (NULL, '$khadamat_id', '$sargrooh_id', '$comboTyp','$now')");
...
///////START sargrooh
	...
	mysql_class::ex_sql("select `reserve_id`,`id` from `room_det` where `reserve_id`>0 and ((date(`aztarikh`) <= '$aztarikh' and date(`tatarikh`) > '$aztarikh') or (date(`aztarikh`) < '$tatarikh' and date(`tatarikh`) > '$tatarikh')) group by `reserve_id`",$q);
	while($r = mysql_fetch_array($q))
	{
		$res_id = $r['reserve_id'];
		mysql_class::ex_sql("select * from `khadamat_cinema` where `reserve_id`='$res_id' and `khadamat_id`='$khadamat_id'",$q_gasht);
...
///////END sargrooh
	...
	$comboTyp = "<select class='inp' id=\"comboTyp\" name=\"comboTyp\"  style=\"width:auto;\">\n<option value=\"-1\">\n&nbsp\n</option>\n";
		if((int)$comboTyp==1)
			...
			echo '';
	        $comboTyp .= "<option value='1' $comboTyp_sobh>نوبت یک</option>\n";
		$comboTyp .= "<option value='2' $comboTyp_asr>نوبت دو</option>\n";
	$comboTyp .= "</select>";
	...
	mysql_class::ex_sql("select * from `room_det` where `reserve_id`>0 and ((date(`aztarikh`) <= '$aztarikh' and date(`tatarikh`) > '$aztarikh') or (date(`aztarikh`) < '$tatarikh' and date(`tatarikh`) > '$tatarikh')) group by `reserve_id`",$q);
	while($r=mysql_fetch_array($q))
	{
		$res_id = $r['reserve_id'];
		mysql_class::ex_sql("select * from `khadamat_cinema` where `reserve_id`='$res_id' and `khadamat_id`='$khadamat_id'",$q_gasht);
		if (!($r_gasht=mysql_fetch_array($q_gasht)))
		{
			...
			if ($h_id==$r_hotel)
			{
				...
				$out.="<tr $row_style >";
				$out .="<td class='showgrid_row_td' >$i</td><td class='showgrid_row_td' >".$hotel_tmp->lname."</td><td class='showgrid_row_td' width='30%'>$rooms</td><td class='showgrid_row_td' >$nafar</td><td class='showgrid_row_td' >".$r['reserve_id']."</td><td class='showgrid_row_td' >".$hotel_tmp->tozih."</td><td class='showgrid_row_td' >".$daftar->name."</td><td class='showgrid_row_td' >".$ajans->name."</td><td class='showgrid_row_td' >".$r['nafar']."</td><td class='showgrid_row_td'>$room_aztarikh1</td><td class='showgrid_row_td'>$room_tatarikh1</td><td class='showgrid_row_td' >خدمات ارائه نشده</td>";
				$out.='</tr>';
				$i++;	
			}
		}
