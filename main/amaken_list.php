<?php 
	...
	if($tmp!='')
	{
		mysql_class::ex_sql("select * from `mehman` where `reserve_id` in ($tmp) and `khorooj`='0000-00-00 00:00:00' and `room_id`>0 order by `room_id`",$qq);
		while($rr=mysql_fetch_array($qq))
		{
			...
			if ($tarikh_mehman!="")
			{
				$t_vorood = perToEnNums(audit_class::hamed_pdate_1($tarikh_mehman[0]));
				$t_khorooj = perToEnNums(audit_class::hamed_pdate_1($tarikh_mehman[1]));	
			}
			else
			{	
				$t_vorood = "0000-00-00";
			...
			if (($t_vorood=="0000-00-00")||($t_khorooj=="0000-00-00"))
			{
				mysql_class::ex_sql("select `aztarikh`,`tatarikh` from `room_det` where `reserve_id`='$r_id' and `room_id`>0",$q_res);
				if($r_res=mysql_fetch_array($q_res))
				{
					$t_vorood = perToEnNums(audit_class::hamed_pdate_1($r_res['aztarikh']));
					$t_khorooj = perToEnNums(audit_class::hamed_pdate_1($r_res['tatarikh']));
				}
			}
			...
			$t_tavalod = perToEnNums(audit_class::hamed_pdate_1($rr["tt"]));
