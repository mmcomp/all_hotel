<?php
	...
	if(!isset($_SESSION['user_id']))
        ...
        if(!$se->can_view)
                die(lang_fa_class::access_deny);
	$daftar_id = new user_class((int)$_SESSION['user_id']);
	$daftar_id = $daftar_id->daftar_id;
	function loadHesab($tbname,$upper_id,$upper_v,$first)
	{
		...
		if($cur_v==-1 && $upper_v == -1 && !$first)
			...
			$cur_v = "";
		mysql_class::ex_sql("select * from `$tbname` where 1=1 $cur_v".(($upper_v == -1)?"":" and `$upper_id` = '$upper_v' order by `name`"),$q);
	...
	<body>
		...
		<div align="center">
			...
			<form id="selfrm" method="POST" <?php echo $action; ?>>
				...
				<input type="hidden" id="sel_id_ghaza" name="sel_id_ghaza" value="<?php echo ((isset($_REQUEST["sel_id"]))?(int)$_REQUEST["sel_id"]:"-1"); ?>" />
