...
function createResult($rooms,$adult,$child,$aztarikh,$paztarikh,$shab,$ghimat){
  ...
  $out .= '<input type="hidden" name="hotel_id" value="'.$hotel_id.'" id="hotel_r" />';
  $out .= '<input type="hidden" name="adult_r" value="'.$adult.'" id="adult_r" />';
  $out .= '<input type="hidden" name="child_r" value="'.$child.'" id="child_r" />';
  $out .= '<input type="hidden" name="aztarikh_r" value="'.$aztarikh.'" id="aztarikh_r" />';
  $out .= '<input type="hidden" name="shab_r" value="'.$shab.'" id="shab_r" />';
...
// 	$out .= '</table>';
  ...
  foreach($rooms as $i=>$room){
		if(count($room['room_ids'])>0){
			...
			$out .= '<tr>';
			$out .= '<td width="20%">';
			$out .= '<div class="room-block-img" >';
			$out .= '<img src="room_pic/room-1.jpg" />';
			$out .= '</div>';
			$out .= '</td>';
			$out .= '<td>';
			$rcount = (int)$room['count'];
			$displayed_rcount = min(3,$rcount);
			$out .= '<div class="room-block-div">';
...
// 	$out .= '<div style="text-align:left">';
  ...
  $out .= '<input type="button" style="margin-left: -16px;margin-top: 20px;" onclick="nextPhase();" value="انتقال به درگاه بانک" />';
	$out .= '</p>';
// 	$out .= '</div>';
// 	$out .= '</form>';
//  $out .= '</div>';
...
if(isset($_REQUEST['StateCode'])){
  ...
  <script>
    $('html, body').animate({
        scrollTop: $("#templatemo_content").offset().top
    }, 2000);
  </script>
SC;
...
$out = str_replace("#aztarikh#",$aztarikh_v,$out);
$out = str_replace("#sargrooh#",$sargrooh,$out);
echo $out;
?>
