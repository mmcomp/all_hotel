...
//echo "SELECT  max(`regdate`) as `tarikh_po` FROM `poshtiban` WHERE `room_id` = $room_id".'<br/>';
		...
		public function loadRoomsByTyp_eslah($hotel_id,$typ,$shart = '',$res)
                {
                        $out = array();
                        $hotel_id = (int)$hotel_id;
                        $room_frase = "";
			...
			$startDate = room_det_class::loadByReserve_startDate($res);
                        mysql_class::ex_sql("SELECT  `id`,`vaziat` FROM `room` WHERE `hotel_id` = $hotel_id $room_frase and `en` = 1 and `room_typ_id`=$typ and (1=1 $shart) order by `room`.`id`",$q);
                        while($r = mysql_fetch_array($q))
			{
				if ($r['vaziat']==5)
				{
...
//echo "SELECT `room_typ_id` , count( `id` ) as `co`,`ghimat` FROM `room` WHERE `zarfiat_max`>=$zarfiat and `hotel_id` = $hotel_id $room_frase and $in_query `en` = 1 $werc group by `room_typ_id`";
		...
		public function loadRooms($hotel_id,$reserve_id = 0)
		{
			...
			mysql_class::ex_sql("SELECT `room_typ_id` , count( `id` ) as `co`,`ghimat` FROM `room` WHERE `hotel_id` = $hotel_id $room_frase and `en` = 1 group by `room_typ_id`",$q);
                        while($r = mysql_fetch_array($q))
                        {
                                $tmp = new room_typ_class((int)$r["room_typ_id"]);
                                $out[] = array("room_typ_id"=>(int)$r["room_typ_id"],"count"=>(int)$r["co"],"name"=>$tmp->name,"ghimat"=>(int)$r["ghimat"],'zarfiat'=>$tmp->zarfiat,'room_ids'=>room_class::loadRoomsByTyp_eslah($hotel_id,(int)$r["room_typ_id"],$shart,$reserve_id));
                        }
                        return($out);
		}	
