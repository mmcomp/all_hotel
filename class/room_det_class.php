<?php
	class room_det_class
	{
		...
		public function loadByReserve_habibi($reserve_id = 0,$room_id = 0)
		{
			...
			{
				$q = null;
				mysql_class::ex_sql("select max(`tatarikh`) as `ta`,min(`aztarikh`) as `az`  from `room_det` where `reserve_id` = '$reserve_id' and `room_id`='$room_id'",$q);
        	                if($r = mysql_fetch_array($q))
                	        {
					$out[0] = $r['az'];
					$out[1] = $r['ta'];
		...
		public function loadRoomByReserve($reserve_id = 0)
		{
			$reserve_id = (int)$reserve_id;
			$out = array();
			if($reserve_id != 0)
		...
		public function loadByReserve_startDate($reserve_id = 0)
		{
			$out = '';
			$reserve_id = (int)$reserve_id;
			if($reserve_id != 0)
			{
				$q = null;
				mysql_class::ex_sql("select `aztarikh` from `room_det` where `reserve_id` = '$reserve_id'",$q);
        	                if($r = mysql_fetch_array($q))
					$out = $r['aztarikh'];
			}
			return($out);
		...
		public function deleteReserve($reserve_id,$sanad,$rooms=TRUE)
		{
			...
			if($rooms)
			{
				mysql_class::ex_sql('select nafar,room_id from room_det where reserve_id = '.$reserve_id,$q);
				while($r = mysql_fetch_array($q))
					$nafars[$r['room_id']] = (int)$r['nafar'];
...
*/
					$reserve_id = (int)$reserve_id;
					$nafar_room = room_det_class::deleteReserve($reserve_id,FALSE,$reserve->editable);
					//var_dump($nafar_room);
					if(!is_array($room_typ_id))
						$rooms = room_class::loadOpenRoomArray($startDate,$delay,$voroodi,$khorooji,$hotel_id,$room_typ_id);
					//----------------KHADAMAT-------------------
					...
					/*
					$cur_tedad = 0;
					foreach($nafar_room as $r_idd => $nroom)
						$cur_tedad += $nroom;
					if($nafar > $cur_tedad)
						$nafar_room[$r_idd] += ($nafar - $cur_tedad);
					else if($nafar < $cur_tedad && $nafar > 0)
					{
						$det = $cur_tedad - $nafar;
						while($det > 0)
							foreach($nafar_room as $r_idd => $nroom)
					...
					*/
					$nafar_room = array($rooms[0]=>$nafar);
					for($i = 0;$i < $tedad && $reserve->editable;$i++)
						if((int)$rooms[$i]>0)
						{
							//echo "insert into `room_det` (`room_id`, `aztarikh`, `tatarikh`, `user_id` , `reserve_id` , `ghimat` ,`nafar`) values ('".$rooms[$i]."','$startDate','$endDate','$user_id',$reserve_id,".(int)($ghimat/(($tedad+$last_tedad)*$delay)).",".((isset($nafar_room[$rooms[$i]]))?$nafar_room[$rooms[$i]]:'0').")";
							mysql_class::ex_sqlx("insert into `room_det` (`room_id`, `aztarikh`, `tatarikh`, `user_id` , `reserve_id` , `ghimat` ,`nafar`) values ('".$rooms[$i]."','$startDate','$endDate','$user_id',$reserve_id,".(int)($ghimat/(($tedad+$last_tedad)*$delay)).",".((isset($nafar_room[$rooms[$i]]))?$nafar_room[$rooms[$i]]:'0').")");
					...
					*/
					...
					$mabchanged = ($mabchanged || $otherchanged);
                                    //    if($mabchanged)
                                   //     {
                                                $senad_record_ids_tmp = new sanad_reserve_class($reserve_id);
                                                $senad_record_ids = $senad_record_ids_tmp->sanad_record;
//var_dump( $senad_record_ids_tmp);
                                                mysql_class::ex_sqlx("delete from `sanad_reserve` where `reserve_id`='$reserve_id'");
                                                //---------------ﺡﺬﻓ ﺍﺯ ﺝﺩﻮﻟ sanad
                                                $q = null;
...
//echo "select `id`,`reserve_id` from `room_det` where `reserve_id`>0 and `room_id` = $room_id and ((`aztarikh` < '$startDate' and `tatarikh` > '$startDate') or (`aztarikh` < '$endDate' and `tatarikh` > '$endDate') or (`aztarikh` >= '$startDate' and `tatarikh` <= '$endDate'))".'<br/>';
		...
		public function roomIdAvailableService($room_id,$startDate,$endDate)
		{
			$out = null;
			$q = null;
			mysql_class::ex_sql("select `id`,`reserve_id`,`service_ezafe` from `room_det` where `reserve_id`>0 and `room_id` = $room_id and ((`aztarikh` < '$startDate' and `tatarikh` > '$startDate') or (`aztarikh` < '$endDate' and `tatarikh` > '$endDate') or (`aztarikh` >= '$startDate' and `tatarikh` <= '$endDate'))",$q);
// 			echo "select `id`,`reserve_id`,`service_ezafe` from `room_det` where `reserve_id`>0 and `room_id` = $room_id and ((`aztarikh` < '$startDate' and `tatarikh` > '$startDate') or (`aztarikh` < '$endDate' and `tatarikh` > '$endDate') or (`aztarikh` >= '$startDate' and `tatarikh` <= '$endDate'))";
			while($r = mysql_fetch_array($q)){
				if($out==null){
					$out = array(
						"reserve_id" => array(),
				...
				$out['reserve_id'][] = $r['reserve_id'];
				$out['service_ezafe'][] = $r['service_ezafe'];
			}
			return($out);
		}
		public function preReserveEzafe($hotel_id,$ajans_id,$room_typ_id,$room_ezafe,$ghimat,$startDate,$delay,$tedad,$voroodi,$khorooji,$nafar,$khadamat,$user_id=-1)
		{
			$conf = new conf;
			$rooms = array();
			$tedad = (int)$tedad;
			...
			$services = array();
			$servuces_data = array();
			foreach($room_ezafe as $rdata){
				$rdata = explode('|',$rdata);
				$services[] = $rdata[0];
				$servuces_data[$rdata[0]] = $rdata[1];
			}
			$other_id_arr = array();
			if(is_array($ghimat))
			{
			...
			if($khorooji)
				...
				$endDate = date("Y-m-d 14:00:00",strtotime($endDate));
			$limit_day = TRUE;
			if((jdate("m/d",strtotime($startDate)) == audit_class::enToPer($conf->limitDate) || jdate("m/d",strtotime($endDate)) == audit_class::enToPer($conf->limitDate)) && $conf->limit_29)
				$limit_day = FALSE;
			...
			if($hotel->hotelAvailableBetween($startDate,$endDate) && $ghimat_darad && $delay>=0 && $room_ok && $limit_day)
			{
				...
				if($tedad <= count($rooms))
				{
					...
					for($i = 0;$i < $tedad;$i++)
					{
						...
						$service_ezafe = 0;
						if(in_array($rooms[$i],$services)){
							$service_ezafe = $servuces_data[$rooms[$i]];
						}
						mysql_class::ex_sqlx("insert into `room_det` (`room_id`, `aztarikh`, `tatarikh`, `user_id` , `reserve_id` , `ghimat` ,`nafar`,`service_ezafe`) values ('".$rooms[$i]."','$startDate','$endDate','$user_id',$reserve_id,$gh_tmp,".(($i==0)?$nafar:'0').",$service_ezafe)");
					}
					//----------------KHADAMAT-------------------
					$khadamat_ghimat = 0;
					if(is_array($khadamat))
...
*/
						mysql_class::ex_sqlx("insert into `room_det` (`room_id`, `aztarikh`, `tatarikh`, `user_id` , `reserve_id` , `ghimat` ,`nafar`) values ('".$rooms[$i]."','$startDate','$endDate','$user_id',$reserve_id,$gh_tmp,".(($i==0)?$nafar:'0').")");
					}
					//----------------KHADAMAT-------------------
					$khadamat_ghimat = 0;
					if(is_array($khadamat))
		...
		public function sabtReserveHotel($reserve_id,$shomare_sanad,$other_ids,$fname,$lname,$toz,$ajans_id,$mhotel,$time_now,$isAdd=TRUE)
		{
			...
			if(is_array($other_ids))
			{
				...
				$other_id_sabti['other_moeen_id3'] = isset($other_ids['other_moeen_id3'])?$other_ids['other_moeen_id3']:-1;
				$other_id_sabti['other_kol_id3'] = isset($other_ids['other_kol_id3'])?$other_ids['other_kol_id3']:-1;
				$other_id_sabti  = serialize($other_id_sabti);
				$mblit1 =(int)$other_ids['ghimat_belit1'];
				$mblit2 =(int)$other_ids['ghimat_belit2'];
				$mblit3 = isset($other_ids['ghimat_belit3'])?(int)$other_ids['ghimat_belit3']:-1;
			}
...
// 			echo "insert into `hotel_reserve` (`fname`,`lname`, `tozih`, `reserve_id`,`ajans_id`,`m_belit1`,`m_belit2`,`m_belit3`,`m_hotel`,`regdat`,`other_id`".((isset($extra_toz))?",`extra_toz`":"").") values ('$fname','$lname','$toz',$reserve_id,$ajans_id,$mblit1,$mblit2,$mblit3,$mhotel,'$time_now','$other_id_sabti'".((isset($extra_toz))?",'$extra_toz'":"").")";
...
/*
			echo ("SELECT `hotel_reserve`.`reserve_id` , `aztarikh` , `tatarikh` 
FROM `hotel_reserve` 
...
AND `room_det`.`reserve_id` >0 $aztar_shart $tatar_shart $shart $name_shart $reserve_id_shart group by `hotel_reserve`.`reserve_id`<br/>\n");
*/
...
AND `room_det`.`reserve_id` >0 $aztar_shart $tatar_shart $shart $name_shart $reserve_id_shart group by `hotel_reserve`.`reserve_id`",$q);
		...
		public function loadReserve($reserve_id,$reserve=null)
		{
			...
			{
				$nafar = 0;
				foreach($reserve->room_det as $rm)
					$nafar += $rm->nafar;
				$out = $reserve->hotel_reserve->lname." تعداد نفرات ".$nafar." نفر از ".audit_class::hamed_pdate($reserve->room_det[0]->aztarikh)." تا ".audit_class::hamed_pdate($reserve->room_det[0]->tatarikh)." جهت ".$hotel->name." ".$khadamat." شماره رزرو $reserve_id";
			}
			else
				$out = 'رزرو شماره '.$reserve_id;
			return($out);
		}
		...
		public function refundReserve($reserve_id,$tozihat='')
		{
			...
			if($reserve->id > 0)
			{
				...
				while($rref = mysql_fetch_array($qref)){
...
//echo "select `reserve_id` from `mehman` where `room_id`='$room_id' and `reserve_id`>0";
		...
		public function loadByReserve_rack($reserve_id = 0)
		{
			$today = date('Y-m-d');
			$out = array();
			$reserve_id = (int)$reserve_id;
			if($reserve_id != 0)
			{
				...
				mysql_class::ex_sql("select `room_id` from `room_det` where `reserve_id` = '$reserve_id' and (date(`aztarikh`) <= '$today' and date(`tatarikh`) > '$today')",$q);
        	                while($r = mysql_fetch_array($q))
					$out[] = $r['room_id'];
			}
			return($out);
