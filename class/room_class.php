<?php
	class room_class
	{
		...
		public function loadOpenRoomsByTyp($startDate,$delay,$voroodi,$khorooji,$hotel_id,$typ,$daftar_id=0)
		{
			...
			if (!($se->detailAuth('admin')))
			{
				$tabaghe = array();
				$werc = '';
				$tabaghe = hotel_garanti_class::loadByDaftarId($daftar_id);
				if (count($tabaghe)>0)
					$werc = ' and `tabaghe` in ('.implode(',',$tabaghe).') ';
				else
				{
					$tabaghe = hotel_garanti_class::loadGaranty();
					if (count($tabaghe)>0)
						$werc = ' and `tabaghe` not in ('.implode(',',$tabaghe).') ';
				}
			}
			else
				$werc = '';
			...
			mysql_class::ex_sql("SELECT  `id`,`vaziat` FROM `room` WHERE `hotel_id` = $hotel_id $room_frase and $in_query `en` = 1 and `room_typ_id`=$typ $werc order by `room`.`id`",$q);
// 			echo "SELECT  `id`,`vaziat` FROM `room` WHERE `hotel_id` = $hotel_id $room_frase and $in_query `en` = 1 and `room_typ_id`=$typ $werc order by `room`.`id`";
//echo "SELECT  `id`,`vaziat` FROM `room` WHERE `hotel_id` = $hotel_id $room_frase and $in_query `en` = 1 and `room_typ_id`=$typ $werc order by `room`.`id`";
			while($r = mysql_fetch_array($q))
			{
				if ($r['vaziat']==5)
				{
					$shart_poshtiban = room_class::loadPoshtiban($r['id'],$startDate);
					if ($shart_poshtiban=='1')
						$out[] = (int)$r['id'];
				}
				else //if($r['vaziat']==2)
					$out[] = (int)$r['id'];
		...
		public function loadPoshtiban($room_id,$tarikh)
                {
                        $out = '';
                        $room_id = (int)$room_id;
                        $room_frase = "";
                        if($room_id > 0)
                        {
				mysql_class::ex_sql("SELECT  max(`regdate`) as `tarikh_po` FROM `poshtiban` WHERE `room_id` = $room_id",$q);
//echo "SELECT  max(`regdate`) as `tarikh_po` FROM `poshtiban` WHERE `room_id` = $room_id".'<br/>';
		                if($r = mysql_fetch_array($q))
				{
		                        if ($r['tarikh_po']<$tarikh)
						$out = '1';
					else
						$out = '0';
				}
		...
		public function loadRoomsByTyp_eslah($hotel_id,$typ,$shart = '',$res,$daftar_id=-1)
                {
			$se = security_class::auth((int)$_SESSION['user_id']);
                        $out = array();
                        $hotel_id = (int)$hotel_id;
			...
			$startDate = room_det_class::loadByReserve_startDate($res);
			if (!($se->detailAuth('admin')))
			{
				$werc = '';
				$tabaghe = hotel_garanti_class::loadByDaftarId($daftar_id);
				...
				$werc = '';
                        mysql_class::ex_sql("SELECT  `id`,`vaziat` FROM `room` WHERE `hotel_id` = $hotel_id $room_frase and `en` = 1 and `room_typ_id`=$typ and (1=1 $shart) $werc order by `room`.`id`",$q);
                        while($r = mysql_fetch_array($q))
			{
				if ($r['vaziat']==5)
				{
		...
		public function loadOpenRooms_old($startDate,$delay,$voroodi,$khorooji,$hotel_id,$zarfiat=0)
		{
			$out = array();
			$delay = (int)$delay;
			$hotel_id = (int)$hotel_id;
...
mysql_class::ex_sql("SELECT `room_typ_id` , count( `id` ) as `co`,`ghimat` FROM `room` WHERE `zarfiat_max`>=$zarfiat and `hotel_id` = $hotel_id $room_frase and $in_query `en` = 1 group by `room_typ_id`",$q);

			while($r = mysql_fetch_array($q))
			{
				$tmp = new room_typ_class((int)$r["room_typ_id"]);
//echo (int)$r["room_typ_id"].'<br/>';
		...
		public function loadOpenRooms($startDate,$delay,$voroodi,$khorooji,$hotel_id,$daftar_id,$zarfiat=0)
		{
			$se = security_class::auth((int)$_SESSION['user_id']);
			$out = array();
			$delay = (int)$delay;
			...
			if (!($se->detailAuth('admin')))
			{
				...
				$werc = '';
mysql_class::ex_sql("SELECT `room_typ_id` , count( `id` ) as `co`,`ghimat` FROM `room` WHERE `zarfiat_max`>=$zarfiat and `hotel_id` = $hotel_id $room_frase and $in_query `en` = 1 $werc group by `room_typ_id`",$q);
//echo "SELECT `room_typ_id` , count( `id` ) as `co`,`ghimat` FROM `room` WHERE `zarfiat_max`>=$zarfiat and `hotel_id` = $hotel_id $room_frase and $in_query `en` = 1 $werc group by `room_typ_id`";
			while($r = mysql_fetch_array($q))
			{
				$tmp = new room_typ_class((int)$r["room_typ_id"]);
				$out[] = array("room_typ_id"=>(int)$r["room_typ_id"],"count"=>(int)$r["co"],"name"=>$tmp->name,"ghimat"=>(int)$r["ghimat"],'zarfiat'=>$tmp->zarfiat,'room_ids'=>room_class::loadOpenRoomsByTyp($startDate,$delay,$voroodi,$khorooji,$hotel_id,(int)$r["room_typ_id"],$daftar_id));
			}
			return($out);
		}
		public function loadRooms($hotel_id,$reserve_id = 0,$daftar_id=-1)
		{
			$se = security_class::auth((int)$_SESSION['user_id']);
                        $out = array();
                        $hotel_id = (int)$hotel_id;
			...
			mysql_class::ex_sql("SELECT `room_typ_id` , count( `id` ) as `co`,`ghimat` FROM `room` WHERE `hotel_id` = $hotel_id $room_frase and `en` = 1 $werc group by `room_typ_id`",$q);
                        while($r = mysql_fetch_array($q))
                        {
                                $tmp = new room_typ_class((int)$r["room_typ_id"]);
                                $out[] = array("room_typ_id"=>(int)$r["room_typ_id"],"count"=>(int)$r["co"],"name"=>$tmp->name,"ghimat"=>(int)$r["ghimat"],'zarfiat'=>$tmp->zarfiat,'room_ids'=>room_class::loadRoomsByTyp_eslah($hotel_id,(int)$r["room_typ_id"],$shart,$reserve_id,$daftar_id));
                        }
                        return($out);
		}
		...
		public function getAnyReserve($tarikh,$room_id=-1)
		{
			...
			if($room_id <= 0)
				$room_id = $this->id;
			$out = room_det_class::roomIdAvailableService($room_id,$aztarikh,$tatarikh);
// 			var_dump($out);
			$services = $out['service_ezafe'];
			$out = $out['reserve_id'];
			$tmp = null;
			for($i = 0;$i < count($out);$i++)
			{
				$hr = new hotel_reserve_class;
				$hr->loadByReserve((int)$out[$i]);
				$tmp[] = array('reserve_id'=>$out[$i],'fname'=>$hr->fname,'lname'=>$hr->lname,'tel'=>$hr->tozih,'service_ezafe'=>$services[$i]);
			}
			$tmp1 = $tmp;
			$ee =array();
			if(count($tmp)>1)
		...
		public function loadTabagheByRoomId($room_id)
		{
			$out = '';
			$id = (int)$room_id;
			mysql_class::ex_sql("select `tabaghe` from `room` where `id` = $id",$q);
			if($r = mysql_fetch_array($q))
				$out = $r['tabaghe'];
			return($out);
		}
		public function loadRoomIdByTabaghe($tabaghe)
		{
			$out = array();
			foreach ($tabaghe as $i)
			{
				mysql_class::ex_sql("select `id` from `room` where `tabaghe` = $tabaghe",$q);
				while($r = mysql_fetch_array($q))
					$out[] = $r['id'];
			}
			return($out);
