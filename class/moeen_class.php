<?php
	class moeen_class
	{
		...
		public function newByCode_habibi($kol_id,$name)
                {
                        mysql_class::ex_sql('select max(`code`) as `co` from `moeen` where `kol_id` = '.$kol_id,$q);
                        if($r = mysql_fetch_array($q))
                        {
                                $code = (int)$r['co'];
                                $code++;
                                mysql_class::ex_sqlx("insert into `moeen` (`kol_id`,`code`,`name`) values (".$kol_id.",$code,'$name')");
                        }
                }
