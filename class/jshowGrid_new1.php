...
*/
				...
				switch($mod){
					case "edit":
						...
						if($this->editFunction!=null){
							...
							if($this->echoQuery)
								$this->query = "update ".$this->tableName." set ".$tmp[2]."=".(($this->field_is_string($tmp[2]))?"'":"").$valu.(($this->field_is_string($tmp[2]))?"'":"")." where ".$this->tableName.".id=$selectedId";
					...
					case "add":
						if($this->addFunction!=null){
							...
							$qur .= (($this->field_is_string($addFields[$i]))?"'":"").$valu.(($this->field_is_string($addFields[$i]))?"'":"").")";
							if($this->echoQuery)
								$this->query = "insert into ".$this->tableName.$qur;
					...
					case "delete":
						if($this->deleteFunction!=null){
							...
							if($this->echoQuery)
								$this->query = "delete from ".$this->tableName." where ".$this->tableName.".id=$selectedId";
		...
		public function executeQuery(){
			if($this->class_enabled){
				...
				if($this->query == ""){
					$this->mysql_class->ex_sql("select ".$this->arrayToString($this->fieldList)." from `".$this->tableName."` ".$this->extraClause." where ".$this->whereClause.$filter,$this->dataSet);
				}else{
		...
		public function getGrid(){
			//$funcName = "khonsa_".$this->gridName;
			function khonsa_pshowgrid($inp){
                        	return($inp);
                	}
/*			$this->rowCount = mysql_num_rows($this->dataset);
			if(($this->rowPerPage>0)&&($this->rowPerPage<$this->rowCount)){
...
Holly;
			...
			$out = $out . "<tr class=\"" . $this->cssClass . "_header\" >\n<th class='".$this->cssClass."_gHead' style=\"width:$index_width;\">\n";
			...
			while($r = mysql_fetch_array($this->dataSet,MYSQL_ASSOC)){
				...
				$i = 0;
				$out = $out . "<td ".(($this->canDelete)?"onmouseover=\"mover_".$this->gridName."('$j');\" onmouseout=\"mout_".$this->gridName."('$j');\"":"")." class=\"" . $this->cssClass . "_row_td\">\n";
...
*/				if($this->canDelete){
			...
			if($this->canDelete)
			{
				$out .= "<input type=\"button\" class=\"" . $this->cssClass . "_insert_button\" value=\"".$this->deleteButton."\" onclick=\"groupDelete_".$this->gridName."();\" ".((!$this->canDelete)?"disabled=\"disabled\"":"")."/>";
			}
			else
			{
				$out .= "&nbsp;";
			...
			$out = $out . "</tr>\n";
			$out = $out . $this->footer;
			$out .= (($this->canAdd)?"<tr class=\"" . $this->cssClass . "_row_even\"><td colspan=\"2\" class=\"" . $this->cssClass . "_row_td\">".$this->newTitle."</td>\n":"");
				...
				if($this->columnLists[$i]==null && $this->columnRLists[$i]==null){
					...
					$ttmp = ((isset($this->newFieldRefresh[$this->fieldList[$i]]))?$this->newFieldRefresh[$this->fieldList[$i]]:'');
					$out = $out . "<select ".(($this->hasRList())?"onchange=\"refresh_combos_".$this->gridName."('".$this->fieldList[$i]."');\"":"")." id=\"new_".$this->fieldList[$i]."\" name=\"new_".$this->fieldList[$i]."\">\n" . $this->columnListToCombo($this->columnLists[$i],$ttmp)."</select>\n";
				}else
				{
					$ref = $this->columnRLists[$i]["ref"];
					$ttmp = ((isset($this->newFieldRefresh[$this->fieldList[$i]]))?$this->newFieldRefresh[$this->fieldList[$i]]:'');
					$tref = ((isset($this->newFieldRefresh[$ref]))?$this->newFieldRefresh[$ref]:'');
					$out = $out . "<select onchange=\"refresh_combos_".$this->gridName."('".$this->fieldList[$i]."');\" id=\"new_".$this->fieldList[$i]."\" name=\"new_".$this->fieldList[$i]."\">\n" . $this->columnRListToCombo($this->columnRLists[$i],$ttmp,$tref)."</select>\n";
				}
                                $out = $out . "</td>\n";
                        }
			$out .= (($this->canAdd)?"</tr>\n":"");
...
//			}
		...
		public function setERequest($inp){
			$this->eRequest = $inp;
                ...
                public function addFeild($feildName)
                {
