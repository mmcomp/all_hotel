...
mysql_class::ex_sql("SELECT  `id`,`vaziat` FROM `room` WHERE `hotel_id` = $hotel_id $room_frase and $in_query `en` = 1 and `room_typ_id`=$typ order by `room`.`id`",$q);
//echo "SELECT  `id`,`vaziat` FROM `room` WHERE `hotel_id` = $hotel_id $room_frase and $in_query `en` = 1 and `room_typ_id`=$typ order by `room`.`id`".'<br/>';
                        while($r = mysql_fetch_array($q))
                        {
//echo $r['id'].'vaz'.$r['vaziat'].'<br/>';
				if ($r['vaziat']==5)
				{
