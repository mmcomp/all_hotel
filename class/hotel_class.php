	...
	class hotel_class
	{
		...
		public function __construct($id=-1)
		{
			...
			if($r = mysql_fetch_array($q))
			{
				...
				mysql_class::ex_sql("select * from `emkanat_hotel_extra` where `hotel_id` = $id",$q);
				if($r = mysql_fetch_array($q)){
					...
					mysql_class::ex_sqlx("insert into `emkanat_hotel_extra` (`hotel_id`) values ($id)");
					mysql_class::ex_sql("select * from `emkanat_hotel_extra` where `hotel_id` = $id",$q);
					if($r = mysql_fetch_array($q)){
						foreach($r as $key=>$value){
							if(!isset($this->$key)){
		...
		public function getProperties(){
			$out = array();
			$sql = "select `emkanat_hotel`.`id` `eid`,`name`,`emkanat_hotel_det`.`id` `edid` from `emkanat_hotel` LEFT OUTER JOIN `emkanat_hotel_det` ON (`emkanat_hotel`.`id`=`emkanat_hotel_det`.`emkanat_hotel_id` and `hotel_id`=".$this->id.") order by `emkanat_hotel`.`id`";
			mysql_class::ex_sql($sql,$q);
